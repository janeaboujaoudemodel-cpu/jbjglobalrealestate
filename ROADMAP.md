...
|---|---|---|---|---|---|
| RM-038 | 2026-08-18 | Claude Code session | JBJ-033 new: added `src/styles/pass-377-dark-host-heading-contract.css`, forcing `#FFFFFF` heading foreground on 8 dark-surface classes (`.jj-hero-fullscreen`, `.jj-hero-neon`, `.jj-cta-primary`, `.jj-pill-emerald`, `.jj-side-tile.is-active`, `.surface-dark`, `.surface-ink`, `.surface-navy`) that were missing from the bright-background guard's `:not()` exclusion chain in `index.css`. Wired via a JS import in `src/main.tsx` after the last `pass-3xx` import — this repo loads pass-N stylesheets that way; `index.css` has zero `@import` at-rules to hook into, despite that being the mechanism initially described for this task. Expanded PASS 142's static-lock test (`REQUIRED_EXCLUSIONS` 10→18) and added a PASS 377 describe block, explicitly flagged in-file as a deletion/drift guard only — it checks the CSS file contains the expected rules, not that contrast actually passes in the rendered DOM. Verified locally: full Vitest suite (40 files, 431 tests, all passing), `check:contrast:pr-gate` (all WCAG AA pairs pass), `check:a11y` (pre-existing baseline, unaffected). **Not yet verified live** — no Playwright run against a deployed preview from this session (no live preview exists yet); that's the real proof and is still outstanding. Originally assigned JBJ-031/RM-034, which collided with `main`'s production-blank-page fix (also JBJ-031) and the JBJ-032 tokens.json cleanup item, both merged first — renumbered to the next actually-free IDs, JBJ-033/RM-038, on merge. | JBJ-033 | `fe0b39b` |
...
| JBJ-033 | Dark-host heading contrast gap-fill — 8 dark-surface classes missing from PASS 142's bright-background `:not()` exclusion chain | **Fixed in code (Aug 18, 2026)**, pushed to `claude/pass-377-dark-host-heading-8rgxpp` (`fe0b39b`), PR #62. `src/styles/pass-377-dark-host-heading-contract.css` forces white headings on `.jj-hero-fullscreen`, `.jj-hero-neon`, `.jj-cta-primary`, `.jj-pill-emerald`, `.jj-side-tile.is-active`, `.surface-dark`, `.surface-ink`, `.surface-navy`. Local Vitest/contrast/a11y checks all pass. **Not verified live** — the claimed "32 failing → 0 failing" Playwright result has not been reproduced by any session; `tests/dark-surface-heading-contrast.spec.ts` needs to run against a deployed preview once this syncs through Lovable before this can be marked Resolved. Originally assigned JBJ-031, which collided with the production-blank-page fix above (also originally JBJ-031) and the JBJ-032 tokens.json cleanup item — renumbered to JBJ-033 on merge | Near-term |
