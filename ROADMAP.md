...
| JBJ-017 | Pass-file `!important` audit (54 `pass-NNN-*.css` files, ~1,962 declarations) | **Stabilised (Aug 18, 2026) — freeze in place, audit itself still not started.** A CI-enforceable ceiling now exists: `scripts/css-guard.mjs` counts `!important` across `src/**/*.css` excluding the compiled `src/index.css` (double-counts otherwise) and fails `--ci` if the count exceeds the frozen baseline of 6,460. `index.css` itself carries a separate ~8,000 `!important`s, out of this guard's scope, not zero. Stylelint (`stylelint` + `stylelint-config-standard`, `.stylelintrc.json`) wired in alongside it — `declaration-no-important` deliberately left off since a hard ban would fail on every existing occurrence with no incremental path. See `docs/CSS_ARCHITECTURE.md`. This stops the count from growing further; it does not touch the original audit scope below (which pass-file overrides are load-bearing vs. dead vs. genuine unresolved conflicts) — that work is unstarted. | Freeze: Resolved. Audit: Future (post-Step 5) |
