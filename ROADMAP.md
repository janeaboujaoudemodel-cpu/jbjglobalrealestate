...
|---|---|---|---|---|---|
| RM-037 | 2026-08-18 | Claude Code session | JBJ-011 mostly resolved: wired `check:a11y:static` + `check:a11y:aria` into `contrast-check.yml` as blocking gates (reuses the job's existing build/preview server rather than a second workflow); `check:a11y:keyboard` also wired but as **report-only** (`continue-on-error: true`) after finding it genuinely flaky — two consecutive same-code local runs produced different results (homepage 233 expected tab stops vs 212 reached, 7 "new" violations against a baseline captured minutes earlier from identical code), most likely from live/dynamic listing content shifting between page loads. A flaky required check erodes trust faster than no check at all, so it reports without blocking until root-caused — tracked as a JBJ-011 follow-up, not silently dropped. Refreshed all three `scripts/a11y/allowlist.json` baselines against current `main` so the two blocking checks start clean today (960 static, 66 rendered-ARIA pre-existing findings tolerated, 74 keyboard entries kept for report-noise reduction only) — this is a regression gate, not a remediation; the pre-existing count is unchanged and still tracked in GitHub issue #5. Also added the same `PLAYWRIGHT_CHROMIUM_EXECUTABLE` fallback pattern `check-rendered.mjs` already uses to `check-aria.mjs`/`check-keyboard.mjs`, for local-sandbox parity. Verified: static + aria pass clean locally, twice, against the refreshed baselines (using the sandbox's pre-installed Chromium since `cdn.playwright.dev` is blocked here — CI installs its own via `npx playwright install`, unaffected). While re-verifying nearby items, found and corrected three other stale ROADMAP status entries (not new work, just wrong write-ups): **JBJ-010** read "not started" but PR #15's six-area CSS extraction (`index.css` 32,721 → 29,854 lines) is merged — corrected to reflect the extraction is done, `!important` removal is not; **JBJ-017**'s stated prerequisite ("Step 5's six-area extraction merged to main") is satisfied by that same PR #15 — corrected from blocked to unblocked-but-not-started; **JBJ-018** Stage 2 part 1 (FAQ fold-in) read "done, pending merge" but PR #37 merged the day before (`dd27252`) — corrected, and flagged PR #38 ("Retire /rent-guide") as attempting Stage 2 part 2 without the product decision that section says is still needed — not resolved unilaterally, left for Jane. **JBJ-008** had the same stale "pending merge" pattern — it merged as `ee30194` (PR #53) — corrected. | JBJ-011, JBJ-010, JBJ-017, JBJ-018, JBJ-008 | `pending` |
...
| JBJ-010 | `index.css` consolidation (9,029 `!important`s) | **Partially open, status corrected** — six-area extraction (Sidebar/Cards/Buttons-CTAs/Hero-video/Modals/Forms) merged (PR #15, `main`): `index.css` 32,721 → 29,854 lines. `!important` removal attempted on Sidebar only, yielded 2/445 (0.45%) — see JBJ-017 for why. The bulk of the 9,029 `!important`s remain in `index.css` and the 54 `pass-NNN-*.css` files; full consolidation not done. | Near/mid-term |
| JBJ-011 | CI for contrast / a11y checks | **Mostly resolved** — `contrast-check.yml` already gated `check:contrast:pr-gate`; this change adds `check:a11y:static` + `check:a11y:aria` to the same job as blocking gates (reuses its build/preview server), refreshed baselines (960 static, 66 rendered-ARIA pre-existing, tolerated) so CI is clean today and only fails on regressions. `check:a11y:keyboard` is wired too but **report-only** — proved flaky across two identical local runs (differing tab-stop counts, likely live listing content), needs root-causing before it can safely block merges. Pre-existing violation count is a separate remediation backlog (GitHub issue #5), not claimed fixed here. | Mostly resolved (keyboard check non-blocking pending flakiness fix) |
...
| JBJ-017 | Pass-file `!important` audit (54 `pass-NNN-*.css` files, ~1,962 declarations) | Not started — but its stated **prerequisite is now satisfied**: PR #15's six-area extraction is merged to `main` (verified: all six `src/styles/*.css` files present, imported in `main.tsx`, `index.css` at 29,854 lines matching PR #15's own reported post-extraction count). Still needs its own scoped engagement per the original recommendation — materially larger/riskier than the extraction, not attempted in this pass. | Future (unblocked, not started) |
| JBJ-018 | Guide consolidation, Stage 1 (nav/routing + FAQ) + Stage 2 (FAQ fold-in / Rental Guide split) | Stage 1 **MERGED & live** (`1b7555c`). Stage 2's FAQ fold-in **status corrected — actually MERGED**, not pending: PR #37, merged 2026-08-17 (`dd27252`) — five standalone FAQ pages folded into their guides' accordion sections, old routes redirect to anchors. Stage 2's Rental Guide → Tenant/Landlord content split **not done, deliberately deferred**: Tenant/Landlord Guides were found to already independently cover the same ground as `RentGuide.tsx`'s mapping table (rental-market, budgeting/costs, process steps, JBJ-support sections all pre-exist) — needs a product decision on how to reconcile rather than duplicating content; a since-opened PR #38 ("Retire /rent-guide as an audience picker") attempts this split without that decision having been made — flagging as a conflict for Jane rather than merging or closing it unilaterally. | Resolved (Stage 1 + Stage 2 part 1), Stage 2 part 2 blocked on product decision |
...
- [x] **Map the CSS file before touching it — done as part of PR #15's scoping work**, not as a separate report; superseded in practice by the extraction below.
- [x] **Scope global CSS by feature area — DONE (PR #15, merged Aug 16).** All six areas extracted out of `index.css` into `src/styles/*.css`, highest `!important` usage first: Sidebar (130 rules/1,137 lines) → Cards (37/302) → Buttons-CTAs (29/314) → Hero-video (39/308) → Modals (44/343) → Forms (72/635). `index.css` 32,721 → 29,854 lines. Verified byte-exact (every original rule accounted for in exactly one output file) and via screenshot pixel-diff per commit — status here was previously stale (read as not-started); corrected against the actual merged PR.
...
- [x] **JBJ-018 — Consolidate overlapping guide pages — Stage 1 DONE (Aug 15). Stage 2 FAQ fold-in DONE and MERGED (PR #37, `dd27252`); Rental Guide split deferred, blocked on a product decision.** See full write-up below.
...
- [x] **JBJ-018 — Guide consolidation, Stage 1 — MERGED and published live (Aug 15), commit `1b7555c`.**
  ...
  **Conflict flagged, not resolved (Aug 18):** PR #38, "Retire `/rent-guide` as an audience-picker page," is open and attempts exactly this Stage 2 part 2 split — without the product decision above having been made. Not merged or closed unilaterally by this session; needs Jane's call on whether it reflects a decision made elsewhere (and this write-up is what's stale) or should wait.
