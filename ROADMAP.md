...
|---|---|---|---|---|---|
| RM-034 | 2026-08-18 | Claude Code session | Found and fixed a tracking-doc bug, not a code bug: JBJ-024/025/026 were still marked "fixed in code, not yet verified live" here, but the actual fix had already merged to `main` via PR #45 (`5eed81e`→`0d06cfd`) — a different branch than the one RM-029 originally referenced (`claude/audit-report-3d-file-w335ta`, which turned out to be a stale/superseded attempt, 10 commits behind `main`, not something to merge). Confirmed the real merge directly: `internal-auth.ts` present, `create-checkout` session-derived, Sentry PII scrubbing + tests present, 5 dead functions removed from the repo. Corrected status to "Resolved (code); live-verify pending" rather than leaving the stale "unmerged" framing. Separately: attempted to pull the PR #43 CI artifact (`actions/runs/32038786479`) to identify JBJ-029's 5 remaining TBD routes — blocked, the artifact's Azure Blob Storage download host isn't in this session's network allowlist; not worked around, flagged as still open. | JBJ-024, JBJ-025, JBJ-026 | `pending`, new PR |
...
| JBJ-024 | Backend audit Phase 0 — real authorization gaps (`create-checkout` unauthenticated; `owner-auth-middleware` role-OR-email disagreement with `verify-owner`; `crm-purge-trash` / `compute-user-scores` ungated) | **Merged to `main`** (PR #45, `5eed81e`→`0d06cfd`) — confirmed the fix is genuinely present in the current tree: `internal-auth.ts` exists, `create-checkout` derives identity from the verified session only. Roadmap status was stale (still said "not yet verified live" after the merge landed); corrected here. **Still not verified against the live deployment** — that check remains open | Resolved (code); live-verify pending |
| JBJ-025 | Backend audit Phase 1 — rollout gap on existing controls (rate limiting, zod validation, static error strings, `email-send-gateway` open relay) | **Merged to `main`** (PR #45, same commit as JBJ-024/026 — one combined PR). Roadmap status corrected — was stale. **Live deployment not verified** | Resolved (code); live-verify pending |
| JBJ-026 | Backend audit Phase 2 — cascade-delete risk, AI-call timeout, Sentry PII scrub, dead deployed functions | **Merged to `main`** (PR #45, same commit) — confirmed `beforeSend`/`beforeBreadcrumb` PII scrubbing present in `src/lib/sentry.ts` with `sentry.test.ts` alongside it, and the 5 dead functions are gone from `supabase/functions/`. Roadmap status corrected — was stale. **Live deployment not verified**; deleted functions may still be deployed, see JBJ-027 | Resolved (code); live-verify pending |
