...
|---|---|---|---|---|---|
| RM-034 | 2026-08-18 | Claude Code session | Stripe integration review (requested as "get started building on Stripe" — the repo already has one). Setup steps 1–3 all failed for environment reasons, not repo reasons: no plugin marketplace is configured in this container, and the agent proxy denies `mcp.stripe.com`, `docs.stripe.com` and `api.stripe.com` outright (403/CONNECT-rejected), so the `stripe_implementation_planner` tool and the `npx skills add https://docs.stripe.com` fallback were both unreachable; review was done by reading the code. Found and fixed 3 defects (JBJ-033 renewal-grant regression, JBJ-034 portal CORS/error-leak/rate-limit + non-constant-time signature compare, undocumented payments env vars). Found and did NOT fix 2 revenue-affecting P0s that need a product decision first (JBJ-031 `user_subscriptions` written by nothing, JBJ-032 no fulfillment for one-time Academy/book SKUs). Verification is limited and explicitly not "green CI": `node_modules` is empty and `npm install` fails on the known `cdn.sheetjs.com` block (RM-025/JBJ-021), so neither Vitest nor the repo's `tsc` ran; the changed files are Deno edge functions outside `src/` and outside both harnesses anyway. Checked with a standalone `tsc --noResolve` parse/type pass — clean apart from the expected Deno `.ts`-extension notes. Nothing here is verified against live Stripe. | JBJ-031, JBJ-032, JBJ-033, JBJ-034 | `pending` |
...
| JBJ-031 | Stripe payments never grant AI-tool access — `user_subscriptions` is written by nothing | **Open, P0, revenue-affecting.** Two unconnected subscription systems exist. `payments-webhook` writes only `subscriptions`; `useSubscription.ts` gates every AI tool via `hasToolAccess()` off `user_subscriptions` + `subscription_tiers`. A repo-wide grep finds no writer for `user_subscriptions` in `src/` or any of the ~507 edge functions — it is read-only everywhere. Net effect: a customer can pay for an AI-tools subscription and `SubscriptionGate`/`AIToolPreviewGate`/`Quiz.tsx` will still deny access. Needs a product decision (repoint the hook at `subscriptions`, or have the webhook write `user_subscriptions`) before it can be coded | Immediate |
| JBJ-032 | One-time purchases (Academy bundles, books, Investor Starter) have no fulfillment path | **Open, P0, revenue-affecting.** `/academy` and `/membership` sell 5 one-time SKUs (`jbj_academy_single/5/10/20_onetime`, `jbj_investor_starter_onetime`) through `StripePricingGrid` → `create-checkout`. The webhook's `checkout.session.completed` branch only calls `handleCreditPackPurchase`, which matches line items against `broker_credit_pack_definitions` and returns silently on no match. There is no `orders`/`purchases`/`entitlements` table anywhere in `supabase/migrations/`. Money is captured; nothing is recorded and nothing is granted. Needs a table + handler, and a decision on what "access" means per SKU | Immediate |
| JBJ-033 | Broker credit renewals silently skipped — `invoice.subscription` removed in Stripe API 2025-04-30.basil | **Fixed in code (Aug 18), not yet verified live.** The client pins `2026-03-25.dahlia`, but `handleInvoicePaid` guarded on the pre-Basil top-level `invoice.subscription`, so every renewal invoice returned early and never refilled the tier's monthly credit allowance. Rewritten to resolve the subscription id from all known payload shapes, then read the price off the retrieved subscription (which still carries the stable `lookup_key` the tier table is keyed on) instead of the invoice line items, whose shape changed in the same release. Not verifiable from this session — all Stripe domains are blocked by the environment's network policy; needs one real renewal, or a Stripe CLI `trigger invoice.paid`, against the deployed function | Immediate |
| JBJ-034 | `create-portal-session` hardening + webhook signature timing leak | **Fixed in code (Aug 18), not yet verified live.** `create-portal-session` had blanket `Access-Control-Allow-Origin: *`, echoed raw exception text to the client, and had no rate limiting — brought onto the same pattern `create-checkout` already uses (`getCorsHeaders`, `enforceRateLimit`, generic error text). Separately, `_shared/stripe.ts` compared webhook signatures with `Array.includes`, which short-circuits on the first differing byte; replaced with a constant-time compare. Also documented the 6 previously-undocumented payments env vars in `.env.example` | Near-term |
