...
|---|---|---|---|---|---|
| RM-035 | 2026-08-18 | Claude Code session | **The 3.1 fix is confirmed working, and it unmasked two things the old contrast runs were not seeing.** A dispatched Contrast Regression Check on the fixed branch head shows four `/owner/*` routes now flagged `unexpectedRedirect` (they redirect to auth for a logged-out sweep) where they previously reported a clean 0 — the router only runs at all now that the bundle boots, which is direct behavioural evidence the charts-vendor fix works. It also surfaced **1 real axe `color-contrast` violation on mobile `/services`**, previously reported as 0. A bundling change cannot alter CSS colours, only whether content renders, so that violation was always present and was masked by a page that never mounted. Logged as JBJ-033. | JBJ-030, JBJ-033 | `fa51b38` |
| RM-034 | 2026-08-18 | Claude Code session | **Finding 3.1 root-caused and fixed — it was reproducible after all.** Wiring the previously-dead regression spec into CI produced a real result: all 5 cases failed with `#root` present and zero children, and the instrumented run named the cause — `Uncaught TypeError: Cannot read properties of undefined (reading 'useState')` in `assets/charts-vendor-*.js`. The hand-assigned `charts-vendor` manual chunk (recharts + d3) evaluated before the React namespace was initialised, throwing pre-mount and tripping index.html's boot overlay, so React never mounted and the page was blank. Production-build only — `vite dev` serves unbundled ESM and does no manual chunking, which is exactly why it never reproduced from source. Removed the hand-assignment so Rollup places these modules. | JBJ-030 | `pending` |
| RM-033 | 2026-08-18 | Claude Code session | Found JBJ-022 was not just cosmetic: the unhandled `createComment` 403 was failing the whole **Contrast PR Gate (blocks merges)** job on runs whose contrast checks had all passed (PR #47's own run: PR gate passed, 31 token pairs 0 fail, 0 axe violations across 56 route/viewport combinations — job still red). Wrapped the summary-comment call in try/catch in `contrast-check.yml` and `pdf-export-qa.yml` so an unpostable comment warns and falls back to the job summary instead of inventing a failure. Both checks' own exit statuses still fail the job, so the gate is unchanged for real regressions. | JBJ-022 | `pending` |
| RM-032 | 2026-08-18 | Claude Code session | Verified rather than repeated the JBJ-030 claim that the 3.1 regression spec was covered by CI — it was not. `tests/owner-shell-renders.spec.ts` was run by no workflow (no `playwright.config.ts` exists, and every Playwright workflow names its spec file explicitly), so the guard never executed. Added `.github/workflows/owner-shell-renders.yml` to build, serve and drive it, and corrected the JBJ-030 wording. | JBJ-030 | `93116ac` |
| RM-031 | 2026-08-18 | Claude Code session | Extended audit finding 3.2 from one function to the defect class: grepped for body-supplied user ids reaching a service-role query. `ai-executive-assistant` checked only that an `Authorization` header existed, then read/wrote another user's private canned-reply templates and spent AI Gateway credits (fixed: caller resolved via `auth.getUser()`, `userId` removed from the contract, rate limit added). `send-idea-approved-email` had no caller check at all and took recipient/name/title from the body into a Resend send — an open relay on the company sending domain (fixed: internal-caller or owner/admin gate). `owner-user-detail` matched the grep but verified correctly and was left alone. | JBJ-032 | `3c793f4` |
| RM-030 | 2026-08-18 | Claude Code session | Media Ingestion Engine Audit (fixes authored Aug 17, landed Aug 18 after rebase onto `main`; renumbered from JBJ-021/022/023 then JBJ-024/025/026, both of which `main` claimed first — PR #39/#42/#43 and PR #45 respectively): fixed 3.2 (unauthenticated `process-drive-upload` impersonation + AI spend), 3.3 (no upload validation — added shared validator with magic-byte sniffing plus bucket `allowed_mime_types`/`file_size_limit`), 3.5 (`rel_media_assets`/`rel_listing_media` still readable by any authenticated user — narrower than the audit's "all twelve", since `20260524022402` had already scoped the other ten in May), 3.6 (seller step tabs silently ignored forward clicks). Investigated 3.1 (blank owner portal) across 3 routes x 3 auth states — not reproducible on current `main`; added a Playwright regression spec and recorded the likely publish-lag explanation rather than claiming a fix. Logged 3.7 as a product decision. | JBJ-029, JBJ-030, JBJ-031 | `ff671a7` |
...
| JBJ-022 | Repo workflow permissions block PR-comment posting | Confirmed via live CI evidence (PR #39) — needs Salim to change `Settings → Actions → General → Workflow permissions` | **Worse than cosmetic (Aug 18):** the unhandled 403 was failing the blocking Contrast PR Gate on runs where every contrast check passed. Code-side mitigation landed (try/catch + job-summary fallback); the underlying permission still needs the manual settings change | Blocked on manual action |
...
| JBJ-029 | Media ingestion audit (Aug 17): unauthenticated `process-drive-upload`, unvalidated listing-media uploads, `rel_media_assets`/`rel_listing_media` readable by any authenticated user, seller step tabs ignoring forward clicks | **Resolved (Aug 17)** — findings 3.2/3.3/3.5/3.6 fixed; `rel-media`/`rel-logos` left public-read deliberately (product decision) | Resolved |
| JBJ-030 | Media ingestion audit 3.1: blank `/owner/*` portal | **Root-caused and fixed (Aug 18).** Not a mystery and not a false report: the `charts-vendor` manual chunk evaluated with React undefined and threw pre-mount, so `#root` stayed empty. Production-build only, which is why three prior investigations against dev servers found nothing. Fix removes the hand-assigned chunk; `tests/owner-shell-renders.spec.ts` now runs against a real production build in CI and is the regression guard | Fixed, pending CI confirmation |
| JBJ-031 | Media ingestion audit 3.7: "Google Drive integration" is URL regex + AI guesswork, not an integration (no `googleapis`/OAuth/Picker code exists) | Logged for a deliberate product decision — lightweight-by-design vs. build real Drive OAuth + Picker | Product decision |
| JBJ-032 | Audit 3.2 defect class: two more edge functions trusting an unverified caller (`ai-executive-assistant` IDOR + AI spend, `send-idea-approved-email` open email relay) | **Fixed in code (Aug 18)** — both orphaned (no `functions.invoke` caller), so no live flow affected. Not covered by tests: no `deno test` harness exists for edge functions in this repo | Resolved in code, unverified by execution |
| JBJ-033 | 1 axe `color-contrast` violation on mobile `/services`, plus four `/owner/*` routes flagged as unexpected redirects in the rendered contrast sweep | **Open, newly visible (Aug 18).** Both were previously reported as clean because the pages were blank — the boot crash in JBJ-030 meant axe had nothing to inspect and the router never redirected. Needs the specific element identified from the `contrast-report` artifact of run 32123766894, and a decision on whether the sweep should expect auth-gated routes to redirect | Near-term |
...
- [x] **JBJ-029 — Media ingestion audit: unauthenticated edge function, unvalidated uploads, open CRM read. RESOLVED (Aug 17).**
  ...
  - **3.5 (Medium, narrower than reported)** The audit said all twelve `rel_*` tables were `SELECT ... USING (true)`. That was their creation state (`20260504141024`) but not the live state: `20260524022402` had already re-scoped ten of the twelve to `rel_is_owner()` back in May. Its table list omitted exactly two — **`rel_media_assets` and `rel_listing_media`** — which were still readable by any authenticated account, exposing every listing's media records (storage paths, titles, kinds). Those two are the real fix; for the other ten the migration only consolidates two identical permissive policies into one canonical `rel_staff_read_*` per table. Confirmed every frontend read of these tables is under `src/pages/owner/*`.
...
- [ ] **JBJ-030 — Media ingestion audit 3.1: blank `/owner/*` portal. NOT REPRODUCIBLE from source; needs a check against the deployed bundle.**
  ...
  **Most likely explanation, and the next step:** publishing to jbj.ae is a separate manual step in Lovable, so production can be running an older bundle than `main`. Re-check the live site after the next publish. `tests/owner-shell-renders.spec.ts` locks the invariant — but note that as originally landed **nothing ran it**: this repo has no `playwright.config.ts`, so there is no default test-directory sweep, and every Playwright workflow names its spec explicitly. The spec was dead code and this sentence was false until `.github/workflows/owner-shell-renders.yml` was added (Aug 18) to build, serve and drive it. A genuine regression is caught by CI now.

- [ ] **JBJ-031 — "Google Drive integration" is link-parsing, not an integration (audit 3.7). Needs a product decision.**
...
- [x] **JBJ-032 — Audit 3.2 is a defect class, not one function. RESOLVED in code (Aug 18).**

  Finding 3.2 named `process-drive-upload`. Treating it as a pattern — a body-supplied user id reaching a service-role query — and grepping for that shape found three candidates.

  - `owner-user-detail` — **not vulnerable.** Verifies the bearer token with `auth.getUser()` and then requires the owner email allow-list or an `owner`/`admin` role before reading another user's record. Taking `user_id` from the body is the intended contract there. Left alone.
  - `ai-executive-assistant` — **vulnerable.** The whole auth check was that an `Authorization` header was present; the token was never validated. It then read `assistant_learned_responses` (a user's private canned reply templates) and `assistant_ignore_rules` for a body-supplied `userId` on a service-role client, updated `use_count`/`last_used_at` on the matched row, and passed caller-supplied text to the AI Gateway on the company key. Caller is now resolved from the token and `userId` has left the request contract; `enforceRateLimit` added at 30 per 10 min per user.
  - `send-idea-approved-email` — **vulnerable, and the worst of the three.** No caller check whatsoever. Recipient address, display name and idea title all came from the request body into a branded JBJ email sent via Resend, so anyone could make jbj.ae send arbitrary "Idea Approved! +50 Points" mail to any address — a phishing vector on the company's own sending domain — while also inserting `user_notifications` / `notifications` rows for an arbitrary `user_id`. Now gated on an internal caller (service-role bearer or `CRM_CRON_SECRET`) or a signed-in owner/admin, reusing `isInternalCaller` and `requireOwnerAuth`.

  Neither vulnerable function has a `functions.invoke(...)` caller anywhere in the repo, so neither fix can regress a live flow. Neither is in `config.toml`'s `verify_jwt = false` list, which offers no protection here for the reason 3.2 already established: the anon key is a valid JWT that ships in the frontend bundle.

  **Evidence:** commit `3c793f4`. **Limit on that evidence:** this repo still has no `deno test` harness for edge functions, so both fixes are verified by reading the shared middlewares' actual signatures and return contracts (`enforceRateLimit(req, config, corsHeaders, userId?)`, `requireOwnerAuth -> { response: null } on success`), not by executing them. Deploying is a Lovable step outside this repo.

- [ ] **JBJ-033 — Findings unmasked by the JBJ-030 boot fix. Open.**

  Fixing the blank-page bug changed what the rendered contrast sweep can see, because it is the first time several of these routes have actually rendered in a bundled build.

  - **1 real `color-contrast` violation, mobile `/services`.** Reported as 0 on every previous run. A manual-chunk change cannot alter CSS colours — only whether content renders — so this violation was always there and the page simply was not painting. Needs the offending element identified from the `contrast-report` artifact on run `32123766894`.
  - **Four `/owner/*` routes now flagged `unexpectedRedirect`** (`/owner`, `/owner/bookings`, `/owner/crm/jbj/home`, `/owner/developers`). They redirect to auth for a logged-out sweep, which is correct behaviour — the sweep simply never saw it before, because the router never ran. The question to settle is whether the sweep's route list should include auth-gated routes at all, or expect the redirect.

  Neither is a regression from the fix. Both are pre-existing conditions that a dead page was hiding, which is the same reason the contrast gate could report a clean sweep across all 56 route/viewport combinations while the owner portal was blank in production.
