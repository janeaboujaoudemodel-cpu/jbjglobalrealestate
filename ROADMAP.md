...
| JBJ-030 | Media ingestion audit 3.1: blank `/owner/*` portal reported in production | **Not reproducible** on `main` across 3 routes x 3 owner-verification states; regression spec `tests/owner-shell-renders.spec.ts` locks the invariant. Most likely a Lovable publish lag serving an older bundle | Needs check against deployed bundle |
