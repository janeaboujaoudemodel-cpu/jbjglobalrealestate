...
| JBJ-024 | Media ingestion audit (Aug 17): unauthenticated `process-drive-upload`, unvalidated listing-media uploads, `rel_media_assets`/`rel_listing_media` readable by any authenticated user, seller step tabs ignoring forward clicks | **Resolved (Aug 17)** — findings 3.2/3.3/3.5/3.6 fixed; `rel-media`/`rel-logos` left public-read deliberately (product decision) | Resolved |
| JBJ-025 | Media ingestion audit 3.1: blank `/owner/*` portal reported in production | **Not reproducible** on `main` across 3 routes x 3 owner-verification states; regression spec `tests/owner-shell-renders.spec.ts` locks the invariant. Most likely a Lovable publish lag serving an older bundle | Needs check against deployed bundle |
| JBJ-026 | Media ingestion audit 3.7: "Google Drive integration" is URL regex + AI guesswork, not an integration (no `googleapis`/OAuth/Picker code exists) | Logged for a deliberate product decision — lightweight-by-design vs. build real Drive OAuth + Picker | Product decision |
...
- [x] **JBJ-024 — Media ingestion audit: unauthenticated edge function, unvalidated uploads, open CRM read. RESOLVED (Aug 17).**
...
- [ ] **JBJ-025 — Media ingestion audit 3.1: blank `/owner/*` portal. NOT REPRODUCIBLE from source; needs a check against the deployed bundle.**
...
- [ ] **JBJ-026 — "Google Drive integration" is link-parsing, not an integration (audit 3.7). Needs a product decision.**
