...
| JBJ-023 | `DocumentStudio.tsx`'s full-screen editor overlay (`zIndex: 2147483000`) sits above shared `Dialog`'s hardcoded z-index (120001, `dialog.tsx`), which can't be overridden by a consumer — any plain `Dialog` rendered while the editor is open is invisible/inert underneath it | **Fixed in code (PR #50), not yet verified live.** `dialog.tsx`'s `DialogOverlay`/`DialogContent` now put their hardcoded `zIndex` before the `style` spread instead of after, so a consumer can override it via the `style` prop (className overrides remain blocked). `AssetLibraryDialog.tsx` gained an optional `contentStyle` prop; `DocumentStudio.tsx` passes `contentStyle={{ zIndex: 2147483647 }}` through it. Note: only `DialogContent`'s z-index was raised for this call site — `DialogOverlay` (the dimming backdrop) still renders at 120000, so the backdrop itself may still sit under the editor's own UI even though the dialog panel now renders correctly on top; flagged, not blocking. Still needs: merge, Lovable publish, and a live check that the signature/stamp picker actually renders above the editor before this is marked Resolved | Near-term |
