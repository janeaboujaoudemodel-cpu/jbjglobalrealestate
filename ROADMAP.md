...
|---|---|---|---|---|---|
| RM-038 | 2026-08-18 | Claude Code session (PR #48) | Reconciled this branch with two other sessions' parallel work on the same audit. Combined rather than chose: the known-violations allowlist (JBJ-029, `db700b4`) and the measurement-race fix (JBJ-036) are complementary — the allowlist decides what blocks CI, the race fix decides whether the input is trustworthy. **Corrected the allowlist's contents**, which had been drawn from a run that was not reproducible: `/mortgage-calculator` was removed because it has no `[data-tool-emerald]` shell and therefore cannot breach (it was never being audited), and `/list-property`, `/property-evaluator` and `/rental-index` were added with measured values. The "7 routes, 5 TBD" in JBJ-029 is resolved — there are 4, confirmed identical across two consecutive runs. My item IDs renumbered a second time (JBJ-029..035 -> JBJ-031..037, RM-030..033 -> RM-034..037) around IDs main claimed in parallel. | JBJ-029, JBJ-036 | `pending` |
...
| JBJ-029 | Tool-emerald color-budget violations — **4** routes (not 7), first found PR #43 (Aug 17) | Open — pre-existing, tracked in `known-violations.json`; CI emits ⚠ (non-blocking). All 4 now confirmed and listed, superseding the "2 of 7 confirmed, 5 TBD" state: the 7 was an artifact of the measurement race fixed in JBJ-036, not a real count | Near-term |
...
- [x] **JBJ-036 — The tool-emerald audit silently passed routes it never measured, and its breach count was not reproducible. RESOLVED (Aug 18).**
  ...
  **Not fixed here:** the three stable breaches this exposes (`/list-property`, `/property-evaluator`, `/rental-index`) all reproduce on clean `origin/main` and are pre-existing champagne-on-emerald surface issues needing a design decision, not a mechanical fix.
