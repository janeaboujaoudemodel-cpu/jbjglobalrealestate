...
| JBJ-031 | Production blank page — circular chunk dependency (`charts-vendor` ⇄ `react-vendor`) | **FIXED (Aug 18, `5e265d8`), verified against a real build in a real browser** — `#root` innerHTML 0 → 400,783 chars. Root cause was Rollup's generated CJS-interop helper hoisted into `charts-vendor`, creating a back-edge `manualChunks` cannot route; fixed by dropping the forced recharts/d3 chunk. Two new CI guards (`npm run check:chunks` + a boot assertion) prevent recurrence. **Not yet confirmed on the deployed site** — needs publishing and a browser check | Fixed, pending deploy |
