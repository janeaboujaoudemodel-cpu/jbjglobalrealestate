...
|---|---|---|---|---|---|
| RM-043 | 2026-08-18 | Claude Code session (PR #48) | Recorded JBJ-039: the rendered contrast sweep had been passing vacuously. It scans a production preview build — the build the JBJ-031 chunk cycle was blanking — so axe found 0 violations because nothing rendered. `main`'s own history dates it precisely: last green `a55fe3d` 09:36:17, first red `44b4988` 09:36:56, which is the blank-page fix itself. The 9 violations are newly visible, not newly introduced, and `main` is red on this check independently of PR #48 (whose blocking PR Gate passes). Also noted a display bug at `check-rendered.mjs:137` where a redirected route prints ✗ beside a 0 count. | JBJ-039, JBJ-031 | `pending` |
| RM-042 | 2026-08-18 | Claude Code session (PR #48) | Merged `main` a fourth time. **Correction of record:** audit finding 3.1 (blank `/owner/*` portal) was reported here as not reproducible; it was real, and PR #59 fixed it (JBJ-031) — a circular chunk dependency between `charts-vendor` and `react-vendor` via a Rollup-generated CJS interop helper. My spec ran against a `vite dev` server, which serves unbundled modules and has no vendor chunks, so it tested an artifact that could not exhibit the bug. JBJ-033 rewritten from "not reproducible" to a record of why the investigation missed it, with the standing lesson that bundling and first-paint symptoms must be reproduced against `npm run build` output. Also took `main`'s version of the three workflow comment-posting fixes (JBJ-022) wholesale — both sessions fixed the same 403 and theirs landed first — and renumbered my items a third time (JBJ-032..038, RM-037..042). | JBJ-031, JBJ-033 | `pending` |
| RM-041 | 2026-08-18 | Claude Code session (PR #48) | Reconciled this branch with two other sessions' parallel work on the same audit. Combined rather than chose: the known-violations allowlist (JBJ-029, `db700b4`) and the measurement-race fix (JBJ-037) are complementary — the allowlist decides what blocks CI, the race fix decides whether the input is trustworthy. Filled in the allowlist's 5 TBD entries with measured values, closing the "2 of 7 confirmed" gap. **Note a wrong turn, corrected in the same PR:** the list was first cut from 7 to 4 on the basis of local runs, removing `/mortgage-calculator` as shell-less; CI showed it does have a shell and does breach, so all 7 are restored with CI-measured values. The original count was right — two agreeing local runs proved determinism within that machine, not full coverage. My item IDs renumbered a second time (JBJ-029..035 -> JBJ-032..037, RM-030..033 -> RM-034..037) around IDs main claimed in parallel. | JBJ-029, JBJ-037 | `pending` |
| RM-040 | 2026-08-18 | Claude Code session (PR #48) | Ran down every red check on PR #48 rather than assuming the merge caused them. Three gates (Contrast PR Gate, PASS 142, PDF exports) were **passing** and only failing on a 403 when posting their PR comment — fixed so the comment can no longer decide the verdict (JBJ-036). The tool-emerald audit was passing routes it never measured and returned 3, 4 and 7 breaches on three runs of the same unchanged tree — fixed the race and made skipped routes visible (JBJ-037). The report-pill snapshot's 5 mobile breaches and the emerald audit's 3 stable breaches were both reproduced on a clean `origin/main` worktree and recorded as pre-existing rather than patched blind (JBJ-038). | JBJ-036, JBJ-037, JBJ-038 | `eef30fa` |
| RM-039 | 2026-08-18 | Claude Code session (PR #48) | Merged `origin/main` into the media-ingestion audit branch. Three resolutions worth recording: the three new migrations were renamed to 1230/1235/1240 so main's `20260817120000_backend_audit_phase2...` keeps its slot and still runs first; the `SellerListing` step tab kept this branch's native `<button>` over main's new `<ClickableDiv>` wrapper, since finding 3.6 needs real focus and keyboard activation rather than a div carrying `role="button"`; and all five item IDs added here were renumbered (JBJ-021..023, 025 -> JBJ-032..032) because they collided with IDs main assigned in parallel, with the obsolete JBJ-024 deleted as a duplicate of main's JBJ-021. Also reverted the temporary `npm ci` fallback from five workflows now that `b343cdf` regenerated the lockfile, and restored `artifacts/contrast/tokens.json`, which had been swept in with only a regenerated timestamp. | JBJ-032, JBJ-033, JBJ-034, JBJ-035 | `3464383` |
| RM-038 | 2026-08-18 | Claude Code session | Media Ingestion Engine Audit, finding 3.4: consolidated the two independently hardcoded owner-email lists onto one server-side table (`owner_email_allowlist`), `rel_is_owner()` now reads it instead of a literal, `user_roles` seeded for the proper mechanism, both paths still accepted so nothing can lock out. The missing piece was the guard — `ownerEmails.drift.test.ts` fails CI the moment the two lists disagree in either direction and names the offending address; verified by planting a drift and watching it fail. Renumbered from a colliding JBJ-024/025 after merging main. | JBJ-035 | `ff671a7` |
| RM-037 | 2026-08-17 | Claude Code session | Media Ingestion Engine Audit (external HTML report, arrived outside git): fixed 3.2 (`process-drive-upload` took `userId` from the request body with no auth check — note Supabase's `verify_jwt` would not have caught this, the anon key is a valid JWT; caller now from `auth.getUser()`, rate limited), 3.3 (listing-media uploads had no validation at all — added a shared validator with magic-byte sniffing plus bucket `allowed_mime_types`/`file_size_limit`, which both buckets lacked), 3.5 (all twelve `rel_*` tables `SELECT USING (true)` — scoped to `rel_is_owner()` after confirming every frontend read is under `src/pages/owner/*`), 3.6 (seller step tabs silently dropped forward clicks). 3.1, the audit's launch-blocker, investigated across 3 routes x 3 owner-verification states and **not reproducible** — recorded as such with a Playwright regression spec rather than claimed fixed. 3.7 logged as a product decision. Item IDs renumbered from a collision with main's JBJ-021..025. | JBJ-032, JBJ-033, JBJ-034 | `ff671a7` |
...
- [x] **JBJ-032 — Media ingestion audit: unauthenticated edge function, unvalidated uploads, open CRM read. RESOLVED (Aug 17).**
  ...
  **Evidence:** commits `ff671a7` and `d5ccfa2` (merge of main); 444/444 vitest and all four contrast PR-gate steps green on the merged tree. `npx tsc --noEmit` reports one error, `src/pages/Spreadsheet.tsx:388` — confirmed pre-existing on `main` and in a file this branch does not touch. `check:a11y` reports 191 static violations; a clean `origin/main` worktree reports the same 191, so none are introduced here.
...
- [x] **JBJ-033 — Media ingestion audit 3.1: blank `/owner/*` portal. REAL, and fixed by another session under JBJ-031. This item exists to record why my investigation concluded otherwise.**

  I investigated 3.1 and reported it **not reproducible**, suggesting the live site was probably serving an older bundle than `main`. That conclusion was wrong. The bug was real, and PR #59 found it: a circular chunk dependency between `charts-vendor` and `react-vendor`, created by a Rollup-generated CommonJS interop helper that `manualChunks` cannot route. `charts-vendor` was modulepreloaded first, evaluated before React initialised, threw `Cannot read properties of undefined (reading 'useState')` at module scope and left `#root` empty. `#root` innerHTML went 0 → 400,783 chars once fixed. See JBJ-031.

  **Why my testing could not have caught it, which is the part worth keeping.** I drove `tests/owner-shell-renders.spec.ts` against a **dev server** (`vite dev`). Vite serves unbundled ES modules in dev — there is no `manualChunks`, no vendor chunks and therefore no chunk cycle. The defect existed only in the production bundle. I was testing an artifact that could not exhibit the bug and reading the pass as evidence of absence.

  Compounding it: I had the right hypothesis in hand ("production may differ from what I am testing") and resolved it in the most comfortable direction — assuming a stale deploy rather than a difference between dev and production builds. The measurement note below was real but became a way to explain away a genuine symptom.

  **Kept, because it is still true:** `#root`'s first two children are legitimately zero-height portal anchors, so inspecting `#root > *` reports `height: 0` on a healthy page. Worth knowing when diagnosing a blank-page report.

  **Standing lesson for this repo:** a bundling, chunking or first-paint symptom must be reproduced against `npm run build` output, never a dev server. `tests/owner-shell-renders.spec.ts` should be pointed at a preview server for exactly this reason. PR #59 added `npm run check:chunks` and an app-boots assertion, which cover this class of bug properly.

- [ ] **JBJ-034 — "Google Drive integration" is link-parsing, not an integration (audit 3.7). Needs a product decision.**
...
- [x] **JBJ-035 — Two independent "who is owner" definitions (audit 3.4). RESOLVED (Aug 17), with the drift guard that was the real gap.**
...
- [x] **JBJ-036 — Three green CI gates were reporting "failure" because posting their PR comment 403'd. RESOLVED (Aug 18).**

  Contrast PR Gate, PASS 142 Heading Contrast and Validate PDF exports all showed red on PR #48 while their actual checks passed. Their own logs carry the verdict: the contrast gate logged `PR Gate: passed (no new contrast regressions)`, 31 token pairs / 0 fail, and 0 violations across all 56 route+viewport rows; PASS 142 logged static guards and the 13-route x 3-viewport rendered sweep both passing. Each job then died in its final `Post PR summary` step on `POST /issues/48/comments` returning 403 `Resource not accessible by integration`.

  Two of the three already declared `issues: write`, with a comment saying it was added for this exact 403. It did not help: the repository caps `GITHUB_TOKEN` below what a job-level `permissions:` block can raise. **That cap is a repository setting and cannot be fixed from a workflow file** — it needs Settings -> Actions -> General -> Workflow permissions, which no Claude Code session can reach.

  What was fixed in code is the consequence. The report is now written to the job summary (which needs no permissions) before the comment is attempted, `createComment` is wrapped in try/catch that warns and continues, and `pull-requests: write` was added — plus a permissions block for `pdf-export-qa.yml`, which had none — so the declarations are right if the cap is lifted. This cannot mask a regression: every check step precedes it and fails the job on its own exit status.

  **Evidence:** commit `eef30fa`; job logs 95638052842 (contrast) and 95637816846 (PASS 142) contain both the passing verdict and the 403.

- [x] **JBJ-037 — The tool-emerald audit silently passed routes it never measured, and its breach count was not reproducible. RESOLVED (Aug 18).**

  `scripts/tool-emerald-audit/run.py` polled six one-second ticks for `[data-tool-emerald]`, then put the breach checks behind `if result["hasShell"]`. A route whose lazy chunk lost that race was recorded with a tick as a pass, having never been measured — `/business-card-scanner` printed a passing tick on a screenshot holding **4,494,758 champagne pixels against a 400 cap**.

  Because the race is timing-dependent, the count was not reproducible. Three runs of the same unchanged `origin/main` tree against the same server scored **3, then 4, then 7** breaches. `/property-measurement` appeared in one run with champagne=422539 and in another with 422555 — the route renders identically; only whether it got audited changed.

  Fixed by replacing the blind polling with one explicit `wait_for_selector`, and by reporting an unmeasured route as `–` skipped rather than `✓` passed, with the skipped routes named in the summary line. Deliberately no `networkidle` wait: several of these pages hold a connection open and never reach idle, so it burned a full timeout per route for no added certainty. Pass/fail semantics are otherwise unchanged — only shell-bearing routes can breach.

  **Evidence the fix holds:** two further back-to-back runs of the same unchanged `origin/main` tree now agree exactly — both `4/4 audited routes breached`, both naming the same 4 breached and same 26 skipped routes, where the unpatched script had produced 3, 4 and 7. Runtime went from ~5 to ~8 minutes, inside the workflow's 20-minute budget. The honest count is that **only 4 of 30 routes were ever actually audited**; the other 26 have no emerald shell and had been silently inflating the pass rate.

  **A correction to an earlier claim in this item.** On the strength of two local runs agreeing, this item originally asserted the real breach count was 4 rather than 7, and `known-violations.json` was cut to match — removing `/mortgage-calculator` on the reasoning that it has no emerald shell and therefore could not breach. CI disproved that: `/mortgage-calculator` reports `shell=True` there and breaches at champagne 252,005 / darkInk 1,767,477. It has a shell; the local sandbox was simply too slow to mount it inside the timeout. `/interior-design-ai` and `/ai-personal-shopper` behaved the same way. The original PR #43 count of 7 was correct, all 7 are restored with CI-measured values, and the lesson is recorded rather than quietly patched: agreement between two runs on one machine demonstrates determinism *within* that environment, not that the environment measured everything.

  **What this means for the fix.** Removing run-to-run variance within an environment still holds and is still worth having. What it does not do is make the audited set identical across machines — whether a lazy-loaded shell mounts inside `SHELL_TIMEOUT_MS` depends on the host. The mitigation is that the not-audited routes are now named in the output every run, so a route dropping out of coverage is visible instead of silently counted as a pass.

  **Not fixed here:** the 7 breaches themselves. All are pre-existing champagne-on-emerald surface issues needing a design decision, not a mechanical fix, and are tracked under JBJ-029.

- [ ] **JBJ-038 — Report pill snapshot: 5 pills render differently in the PDF than in the preview, at mobile width only. Pre-existing on `main`.**

  The `Report Pill Snapshot` check fails with 5 breaches at 414px; desktop and tablet are clean. Two "Match" pills on page 3 diverge by a mean per-channel colour delta of **156.1 and 154.6** against a tolerance of 14 — close to inverted, not a rounding artifact. Three "Review" pills sit just over at 18–20.

  **Confirmed pre-existing, not introduced by PR #48:** reproduced identically (5 breaches, mobile only) against a clean `origin/main` worktree. Left open rather than patched because it is a report-styling question outside that PR's scope, and the fix should be driven by looking at the uploaded `report-pill-snapshot` side-by-side PNGs.

- [ ] **JBJ-039 — The rendered contrast sweep was passing because the site was blank. 9 real violations are now visible on `main`.**

  `contrast-check.yml`'s Layer 2 sweep builds the app and runs axe against a **production preview**. That is the same production bundle the circular chunk dependency (JBJ-031) was blanking. Axe reported 0 violations because there was no rendered content to violate anything — the guard was green because it was measuring an empty page.

  **Evidence is a clean before/after on `main` itself.** The last green run is `a55fe3d` at 09:36:17. The first red run is `44b4988` at 09:36:56 — the commit that fixed the blank page. Every run since (`86a878e`, `99ec78e`) is red with the same 9 violations. `main` is currently red on this check.

  So the 9 violations are newly **visible**, not newly **introduced**. They have presumably been shipping for as long as the chunk cycle existed.

  **Not caused by PR #48, and not fixed there.** PR #48's own run shows `gateOutcome = 'success'` — the blocking PR Gate passes; only the non-blocking Layer 2 sweep is red, identically to `main`. Fixing the 9 requires colour decisions on the affected routes, which is a design call.

  **A second, smaller bug in the same script.** `check-rendered.mjs:137` prints `✗` when `filtered.length === 0 && !unexpectedRedirect` is false, so a route that merely redirected prints `✗ … — 0 violation(s)` — a failure marker next to a zero count. Those redirects contribute nothing to `totalViolations` (line 160), so they do not affect pass/fail, but they make the output read as though clean routes are failing. Worth separating "redirected" from "violated" in the output.
