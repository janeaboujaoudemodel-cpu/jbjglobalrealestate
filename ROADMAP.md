...
| JBJ-029 | Lovable preview boot error — `charts-vendor` chunk reads `useState` off an undefined React | **New (Aug 18). Pre-existing, NOT caused by PR #45 — but confirmed to still reproduce on the current build.** Full boot failure: `Uncaught TypeError: Cannot read properties of undefined (reading 'useState')` at `assets/charts-vendor-CK7T8wxi.js:9:132`. Evidence across two builds: first seen on the `3644d5cf` preview (i.e. `main` *before* PR #45), then reproduced on the freshly-generated `0d06cfd5` preview after the merge — **same chunk hash `CK7T8wxi` in both**, which both proves the bundle is byte-identical across the two commits (PR #45 changes no bundler config and did not cause this) and proves the bug is still present in what is deployed now. Classic Rollup chunk-ordering hazard: `vite.config.ts`'s `manualChunks` routes recharts/d3 to `charts-vendor` and React to `react-vendor`, and `charts-vendor` evaluates before React is initialised. **Still not confirmed against the published site** — the session's network policy blocks `jbj.ae`/`lovable.app`, so the evidence is the Lovable preview only. **Could not be fixed or tested from this session**: a production build is impossible here because `xlsx` is CDN-pinned to the blocked `cdn.sheetjs.com` (same constraint as JBJ-021), so a chunking change could not be verified and shipping one blind was refused. Conservative fix to try in an environment that can build: drop recharts/d3 from `manualChunks` and let Rollup place them, or fold them into `react-vendor` | Immediate — site may be down |
