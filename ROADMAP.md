...
**4 tables with RLS enabled but ZERO policies — VERIFIED: service-role-only by design, not silently broken**

- jbj_booking_audit_log, jbj_booking_email_verifications, owner_calendar_api_clients, and webauthn_challenges all have RLS enabled with no policies defined — confirmed directly against the migrations that created them (20260705111819, 20260724193104, 20260812061811), and confirmed no later migration ever added one, across all 1,154 migration files in the repo.
- Traced every real code path touching each table (searched supabase/functions/ and src/, not just table names): webauthn_challenges is read/written exclusively by the service-role admin client across all 4 WebAuthn edge functions (register/auth options + verify); jbj_booking_audit_log and jbj_booking_email_verifications are touched exclusively by the service-role sb client in booking-public-create; owner_calendar_api_clients is touched exclusively by the service-role db() client in owner-calendar-api. No direct frontend/anon-key query to any of the four exists anywhere in src/.
- service_role bypasses RLS entirely regardless of policy count (same DB-level fact already noted under RLS in CLAUDE.md) — so zero policies on a table that's only ever accessed via service_role is the intended design, not a bug. webauthn_challenges even carries an explicit migration comment confirming this: "Intentionally no policies for anon/authenticated: service role only."
- Correction to this item's original framing: booking confirmations and WebAuthn login are not failing silently — both flows are service-role-only end to end, so RLS was never actually positioned to reject them. owner_calendar_api_clients also isn't part of either of those two features; it's a separate, unrelated one (owner calendar API-key management) that happens to share the same zero-policy pattern — also confirmed service-role-only.
- Caveat, not fully closed out: this confirms today's known code paths are safe, not that they'll stay that way. It doesn't guard against a future change adding a user-scoped/anon-key query against one of these four tables — at that point the zero-policy state would start mattering for real. Worth a quick recheck any time new code touches one of these four table names.
