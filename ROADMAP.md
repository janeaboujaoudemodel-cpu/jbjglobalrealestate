...
| JBJ-029 | Lovable preview boot error — `charts-vendor` chunk reads `useState` off an undefined React | **New (Aug 18), pre-existing, not caused by PR #45.** Lovable's project screenshot shows a full boot failure: `Uncaught TypeError: Cannot read properties of undefined (reading 'useState')` at `assets/charts-vendor-CK7T8wxi.js:9:132`. The screenshot's own URL pins it to the `3644d5cf` build — i.e. `main` *before* PR #45 merged — and PR #45 touches no bundler config. Classic Rollup chunk-ordering hazard: `vite.config.ts`'s `manualChunks` routes recharts/d3 to `charts-vendor` and React to `react-vendor`, and `charts-vendor` evaluates before React is initialised. **Not confirmed on the published site** — this session's network policy blocks `jbj.ae`/`lovable.app`, so only the preview screenshot is evidence. Needs a real browser check of the live site first, then a chunking fix (simplest: fold recharts into `react-vendor`, or drop the manual chunk for it) | Immediate — verify live before anything else |
