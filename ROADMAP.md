...
| JBJ-032 | `developers` table — anon-readable PII/internal columns (`admin_email`, `office_phone`, `whatsapp`, `notes`, `enrichment_source`, `last_enriched_at`) | **Fixed in migration `20260818120000`** — re-asserts a column-level `REVOKE`/`GRANT` restriction for the `anon` role using a maintained blocklist. This is the third time this exact restriction has been applied (see `20260524113700`, `20260710100918`→`20260720171520`); it was twice silently undone by an unrelated full-table `GRANT SELECT ON public.developers TO anon`. Frontend already only reads safe columns (`DEVELOPERS_PUBLIC_SELECT`, `src/hooks/useProjects.ts`) — no app-code changes needed. **Not verified live** — no Supabase credentials in this session; only migration history and frontend call sites were checked, not a real anonymous request against the deployed API. `tsc --noEmit`/`npm test` could not be run either (see RM-036 — `npm install` blocked on `xlsx`'s CDN dependency, pre-existing and unrelated) | Fixed in code, unverified live |
| JBJ-033 | `hr_job_offers` — anon-readable `salary_range_min`/`salary_range_max`/`commission_structure` via `"Public can view active job offers"` | **Flagged, not fixed — needs Salim's call** on whether salary ranges are meant to be public (common on job boards) vs. `commission_structure` (more likely unintentional, internal comp-plan detail — confirm rather than assume). Note: migration `20260625191542` already revoked anon SELECT on exactly these three columns and nothing since re-grants them, so this may already be resolved on the live DB — this session has no way to independently confirm live state | Blocked on Salim (product decision) |
