...
| JBJ-024 | Backend audit Phase 0 — real authorization gaps (`create-checkout` unauthenticated; `owner-auth-middleware` role-OR-email disagreement with `verify-owner`; `crm-purge-trash` / `compute-user-scores` ungated) | **Fixed in code (Aug 17), not yet verified live.** `create-checkout` now derives `metadata.userId` from the verified session only; `owner-auth-middleware` is email-allow-list-only, matching `verify-owner`; both cron functions require an internal caller via new `_shared/internal-auth.ts` | Immediate |
| JBJ-025 | Backend audit Phase 1 — rollout gap on existing controls (rate limiting, zod validation, static error strings, `email-send-gateway` open relay) | **Fixed in code (Aug 17), not yet verified live.** Rate limiting added to `create-checkout`, `register-user`, `booking-public-create`; zod added to `register-user`; raw `error.message` replaced with static strings across 6 functions; `email-send-gateway` now enforces a sender-domain allow-list and restricts non-staff callers to self/JBJ recipients | Near-term |
| JBJ-026 | Backend audit Phase 2 — cascade-delete risk, AI-call timeout, Sentry PII scrub, dead deployed functions | **Fixed in code (Aug 17), not yet verified live.** `projects.developer_id` FK moved CASCADE→SET NULL (migration `20260817120000`); 60s AbortController timeout on both AI-gateway paths; `beforeSend`/`beforeBreadcrumb` PII scrubbing in `src/lib/sentry.ts` (8 new unit tests); 5 dead functions deleted from the repo — **note: deleting the directory does not undeploy them, see JBJ-027** | Near-term |
| JBJ-027 | Deployed-but-deleted edge functions still reachable | **Open, needs action outside this repo.** The 4 smoke-test functions and `bulk-import-devs-2027-6` were removed from `supabase/functions/` in the Phase 2 commit, but a repo deletion does not remove an already-deployed function from the Supabase project. Until they are explicitly undeployed, they remain publicly callable — `bulk-import-devs-2027-6` in particular does unauthenticated service-role writes into `developers` | Immediate |
| JBJ-028 | Backend audit Phase 3 — ~210 edge functions with no traceable caller | **Open, not started.** Audit's own estimate, acknowledged as an overestimate (it can't see function-to-function calls, dynamic invoke strings, or webhook targets configured in external dashboards). Needs a per-function pass: confirm dead and remove, or confirm externally wired and document | Mid-term |
| JBJ-029 | Tool-emerald color-budget violations — 7 routes, first found PR #43 (Aug 17) | Open — pre-existing, tracked in `known-violations.json`; CI emits ⚠ (non-blocking). All 7 now identified and listed with measured values, closing the original "2 of 7 confirmed, 5 TBD" gap | Near-term |
...
- [x] **JBJ-031 — Media ingestion audit: unauthenticated edge function, unvalidated uploads, open CRM read. RESOLVED (Aug 17).**
...
- [ ] **JBJ-032 — Media ingestion audit 3.1: blank `/owner/*` portal. NOT REPRODUCIBLE from source; needs a check against the deployed bundle.**
...
- [ ] **JBJ-033 — "Google Drive integration" is link-parsing, not an integration (audit 3.7). Needs a product decision.**
...
- [x] **JBJ-034 — Two independent "who is owner" definitions (audit 3.4). RESOLVED (Aug 17), with the drift guard that was the real gap.**
...
- [x] **JBJ-035 — Three green CI gates were reporting "failure" because posting their PR comment 403'd. RESOLVED (Aug 18).**
...
- [x] **JBJ-036 — The tool-emerald audit silently passed routes it never measured, and its breach count was not reproducible. RESOLVED (Aug 18).**
...
- [ ] **JBJ-037 — Report pill snapshot: 5 pills render differently in the PDF than in the preview, at mobile width only. Pre-existing on `main`.**
