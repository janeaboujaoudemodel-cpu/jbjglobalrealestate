...
| JBJ-021 | `npm ci` lockfile desync + override conflicts | **Partially resolved (PR #39)** — override conflicts (react-hook-form, recharts) and brace-expansion/12+ other package desyncs fixed. A third desync (`xlsx`) discovered mid-fix, not resolved — see full write-up below | In progress |
...
### JBJ-021 — `npm ci` lockfile desync + override conflicts — PARTIALLY RESOLVED (Aug 17, PR #39)
...
**Root cause #3 — `xlsx` desync — NOT RESOLVED, discovered mid-fix.** Getting past root cause #1 unmasked a third, pre-existing desync: the lockfile pins `xlsx@0.18.5`, but `package.json` wants the CDN-hosted `xlsx@0.20.2` (same run of Aikido commits). Confirmed live on PR #39's own CI. **Could not be fixed in the Claude Code cloud session that did this work**: `xlsx@0.20.2` is distributed only via SheetJS's own CDN (`cdn.sheetjs.com`) — the npm registry's latest published version is still `0.18.5`, so no registry mirror has `0.20.2` either. That session's sandbox network policy blocks `cdn.sheetjs.com` outright (403 from the proxy itself, a policy denial rather than an npm error) — and fabricating a lockfile entry (resolved URL, integrity hash, transitive deps) without actually fetching the real tarball was correctly judged not an acceptable workaround.

**What's needed to close this out:** run `npm install` from an environment with real access to `cdn.sheetjs.com` (a local machine, or a differently-configured session) and push the resulting `package-lock.json` — should only touch the `xlsx` subtree, since everything else is already correctly synced by PR #39. Alternatively, a product decision to pin `xlsx` to a registry-available version instead (not this session's call to make unilaterally).
