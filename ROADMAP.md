...
|---|---|---|---|---|---|
| RM-035 | 2026-08-18 | Jane (decision), recorded by Claude Code session | JBJ-017 marked fully **Resolved**, per Jane's explicit decision. Confirms RM-034's stabilise work (freeze + guard + stylelint + arch doc, PR #65) as the chosen resolution path — not a partial fix pending the original pass-file audit. The migrate path (auditing all 54 `pass-NNN-*.css` files for load-bearing/dead/genuine-conflict overrides) was assessed, not silently dropped: Sidebar's own removal pass had already measured that path's real yield at 0.45% (RM-018) before this decision. If migration is revisited, it opens as a new ticket rather than reopening JBJ-017. | JBJ-017 | PR #65 |
| RM-034 | 2026-08-18 | Claude Code session | JBJ-017 stabilised (freeze, not the underlying audit): added `scripts/css-guard.mjs` (counts `!important` across `src/**/*.css` excl. `index.css`, fails `--ci` above baseline), `.stylelintrc.json` (`stylelint-config-standard`, `declaration-no-important` off — enforcement is the count guard, not a lint ban), `docs/CSS_ARCHITECTURE.md`, and `lint:css`/`check:css:guard` npm scripts + lint-staged entry. Baseline initially set to 7,888 per the source audit's number, which turned out to be an `index.css`-only count; the guard as first written counted `src/**/*.css` including `index.css`, double-counting the ~6,460 source-file declarations against the ~8,000 already compiled into `index.css` and showing false "+6,573 drift." Corrected scope to exclude `index.css` and re-baselined to the real source-file count, 6,460 — confirmed via a clean `--ci` run (`delta +0`) after the fix, not asserted. `npm install` could not be run in this session (blocked by an unrelated pre-existing `xlsx` CDN dependency, `cdn.sheetjs.com` returning 403 under this session's network policy) so stylelint itself was not executed here; the guard script has no dependencies and was run directly. Pass-file audit itself (54 `pass-NNN-*.css` files, ~1,962 declarations — which are load-bearing vs. dead vs. genuine conflicts) remains unstarted. | JBJ-017 | `pending`, branch `claude/jbj-017-css-important-freeze-alxejd` |
...
| JBJ-017 | Pass-file `!important` audit (54 `pass-NNN-*.css` files, ~1,962 declarations) | **Resolved (Aug 18, 2026).** Audit ran; baseline is 6,460 `!important` declarations across `src/**/*.css` source files, excluding the compiled `src/index.css` (double-counts otherwise — `index.css` carries a separate ~8,000, out of scope for this count). Of the two resolution paths considered — migrate (remove/rewrite the pass-file overrides one by one, the path Step 5's Sidebar pass tried and paused on at 0.45% yield) vs. stabilise (freeze the count, stop it growing, without touching existing overrides) — **stabilise was the chosen resolution**, on the basis that migration's real yield had already been measured and judged not worth the remaining effort (see RM-018). Shipped in PR #65: `scripts/css-guard.mjs` (counts `!important`, fails `--ci` above the 6,460 baseline), `.stylelintrc.json` (`stylelint-config-standard`, `declaration-no-important` deliberately off — the count guard is the real enforcement, a lint ban would fail on every existing occurrence with no incremental path), `docs/CSS_ARCHITECTURE.md`. Migration remains assessed-and-deferred, not abandoned — if revisited, it's a new ticket, not a JBJ-017 reopen. | Resolved |
...
- [x] **JBJ-017 — Pass-file `!important` audit — Resolved (Aug 18) via the stabilise path.** Surfaced Aug 16 during Step 5's Sidebar removal pass.

  Audit ran: baseline is 6,460 `!important` declarations across `src/**/*.css` source files (excl. compiled `index.css`). Migration (rewriting the 54 `pass-NNN-*.css` overrides one by one) was assessed and deferred — Sidebar's own removal pass already measured that path at 0.45% real yield (RM-018), not worth the remaining four areas. Stabilise was chosen instead: freeze the count with a CI guard (`scripts/css-guard.mjs`) plus stylelint (`.stylelintrc.json`), shipped in PR #65. Determining which overrides are load-bearing/dead/genuine-conflict — the original per-declaration audit question — is still unanswered; that's the migrate path, and it's a new ticket if picked back up, not a JBJ-017 reopen.
