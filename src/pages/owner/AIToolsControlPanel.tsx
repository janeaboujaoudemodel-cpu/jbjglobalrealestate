...
export default function AIToolsControlPanel() {
  ...
  // ─── Render ────────────────────────────────────────────────────────────────
  return (
    <div className="min-h-screen bg-gradient-to-br from-[hsl(32,28%,13%)] via-[hsl(33,27%,15%)] to-[hsl(33,28%,11%)] text-zinc-900 p-4 md:p-8">
      {/* Header */}
      <div className="mb-8">
                    ...
                    <CardContent className="pt-0 pb-4">
                      <Tabs defaultValue="fixes" className="w-full">
                        ...
                        <TabsContent value="fixes">
                          {toolRecs.length === 0 ? (
                            <div className="text-center py-8 text-zinc-600 text-sm">No pending recommendations for this tool.</div>
                          ) : (
                            <div className="space-y-3">
                              {toolRecs.map(rec => (
                                <FixCard
                        ...
                        <TabsContent value="history">
                          {toolVersions.length === 0 ? (
                            <div className="text-center py-8 text-zinc-600 text-sm">No version history yet.</div>
                          ) : (
                            <div className="space-y-2">
                              {toolVersions.map(v => (
                                <div key={v.id} className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-3">
                        ...
                        <TabsContent value="tests">
                          {toolTestLogs.length === 0 ? (
                            <div className="text-center py-8 text-zinc-600 text-sm">No test logs yet.</div>
                          ) : (
                            <div className="space-y-2">
                              {toolTestLogs.map(t => (
                                <div key={t.id} className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-3 flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-2">
                                    {t.tool_url && <code className="text-[10px] text-zinc-600 font-mono">{t.tool_url}</code>}
                                    <span className="text-[10px] text-zinc-600">{new Date(t.created_at).toLocaleString()}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
