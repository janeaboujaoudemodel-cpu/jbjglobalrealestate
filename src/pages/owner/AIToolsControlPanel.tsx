...
const STATUS_CONFIG: Record<string, { label: string; className: string }> = {
  published: { label: "Live", className: "jj-surface-emerald-soft text-[color:var(--emerald-on)] border-[color:var(--emerald-1)]/30/40" },
  live: { label: "Live", className: "jj-surface-emerald-soft text-[color:var(--emerald-on)] border-[color:var(--emerald-1)]/30/40" },
      ...
      <div className="flex items-center gap-3 mb-3 px-3 py-2 rounded-lg bg-[#FDFBF7]/60 border border-[#1A1A1A]/30">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Button size="sm" variant="outline" disabled={selected.size === 0}
            ...
            className="h-7 text-xs border-[color:var(--emerald-1)]/30/50 text-[color:var(--emerald-on)] hover:jj-surface-emerald-soft gap-1">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <div onClick={e => e.stopPropagation()} className="inline-flex rounded-md border border-[#1A1A1A]/40 overflow-hidden">
                            {(["public", "owner_only", "hidden"] as const).map((v) => {
                              const active = vis === v;
                              const meta = v === "public"
                                ? { Icon: Globe, label: "Public", active: "jj-surface-emerald-soft text-[color:var(--emerald-on)]" }
                                : v === "owner_only"
                        ...
                        <TabsContent value="fixes">
                          ...
                          {(status === "applied" || status === "tested") && (
                            <div className="mt-4 flex gap-2 border-t border-[#1A1A1A] pt-4">
                              ...
                              {status === "tested" && (
                                <Button size="sm" onClick={() => publishVersion(tool.id)} className="jj-surface-emerald hover:jj-surface-emerald text-white gap-1">
                                ...
                                <div key={v.id} className="bg-[#F7F2EA]/50 border border-[#1A1A1A]/50 rounded-lg p-3">
                                  ...
                                  {(v.before_snapshot || v.after_snapshot) && (
                                    <div className="grid grid-cols-2 gap-2 mt-2">
                                      ...
                                      {v.after_snapshot && (
                                        <div className="jj-surface-emerald-soft border border-emerald-900/30 rounded p-2">
    ...
    <div className="bg-[#F7F2EA]/40 border border-[#1A1A1A]/40 rounded-lg p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {(isApplied || isTested) && (
            <Button size="sm" onClick={onPublish} className="h-7 text-xs jj-surface-emerald hover:jj-surface-emerald text-white gap-1">
