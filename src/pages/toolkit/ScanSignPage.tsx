/**
 * Scan & Sign Tool — Premium Green Design
 * Camera document scanning (auto/manual), edge cropping, signature, AI enhance, Save/Clear/Delete project
 */
...
const G = {
  ...
  btnPrimaryShadow: "0 4px 20px rgba(16,185,129,0.4)",
  btnOutline: { background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.35)", color: "#fff" },
  outerBorder: "1px solid rgba(16,185,129,0.25)",
  outerShadow: "0 0 60px rgba(16,185,129,0.06), 0 2px 40px rgba(0,0,0,0.6)",
};
...
const PrimaryBtn = ({ onClick, disabled, children, className = "" }: {
  ...
  <button
    onClick={onClick}
    disabled={disabled}
    className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm text-white transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed ${className}`}
...
export default function ScanSignPage() {
      ...
      reader.onload = (ev) => {
        setPages(prev => {
          const newPages = [...prev, {
            ...
            rotation: 0, brightness: 110, contrast: 115,
          }];
          setSelectedPageIndex(newPages.length - 1);
          return newPages;
        });
  ...
  const exportToPDF = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${projectName.replace(/[^a-z0-9]/gi, '_')}_${Date.now()}.pdf`;
      a.click();
  ...
  return (
    <div className="min-h-screen" style={{ background: G.bg }}>
      ...
      {/* ── Header — Premium Green, Tall ── */}
      <div style={{
        background: "linear-gradient(180deg, rgba(16,185,129,0.13) 0%, rgba(16,185,129,0.03) 100%)",
              ...
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold"
                    style={{ background: "rgba(16,185,129,0.15)", border: "1px solid rgba(16,185,129,0.35)", color: G.accentLight }}>
      ...
      {/* ── Main Content — wrapped in outer border ── */}
        ...
        >
          <div className="grid lg:grid-cols-3 gap-6">
              ...
              <Panel>
                ...
                {isCapturing ? (
                  <div className="space-y-3">
                    ...
                    <div className="relative rounded-xl overflow-hidden" style={{ border: `1px solid ${G.border}` }}>
                      <video
                        ...
                        style={{ display: 'block', minHeight: 160, background: '#000' }}
                      />
                      {/* Corner guides */}
                      ...
                      {autoScanCountdown > 0 && (
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="text-7xl font-black" style={{ color: G.accentLight, textShadow: '0 0 30px rgba(52,211,153,0.7)' }}>
                            {autoScanCountdown}
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex gap-2">
                      <PrimaryBtn onClick={capturePhoto} className="flex-1 text-xs">
                        <Camera className="w-4 h-4" /> Capture
                      </PrimaryBtn>
                      <OutlineBtn onClick={startAutoScan} disabled={cameraMode === 'auto'} className="flex-1 text-xs">
                        <ScanLine className="w-4 h-4" /> Auto (3s)
                      </OutlineBtn>
                    </div>
                    <OutlineBtn onClick={stopCamera} className="w-full text-xs">
                      <X className="w-4 h-4" /> Close Camera
                    </OutlineBtn>
                    <p className="text-[11px] text-center" style={{ color: "rgba(255,255,255,0.3)" }}>
                      Tip: Use <strong>Auto Scan</strong> for hands-free capture
                    </p>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    ...
                    <div
                      className="w-full rounded-xl p-5 text-center cursor-pointer transition-all"
              ...
              <Panel>
                  ...
                  {pages.map((page, index) => (
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: selectedPageIndex === index ? `0 0 0 2px rgba(16,185,129,0.2)` : "none",
                      }}
                    >
                      <img
                        src={page.imageData}
                      ...
                      <div className="absolute bottom-0 left-0 right-0 px-1 py-0.5 flex items-center justify-between"
                        ...
                        {signatures.some(s => s.pageIndex === index) && (
                          <Pen className="w-2.5 h-2.5" style={{ color: G.text }} />
                        )}
                      </div>
                ...
                {pages.length > 0 && (
                  <p className="text-[11px] text-center mt-3" style={{ color: "rgba(255,255,255,0.3)" }}>
                    Click a page to select · Tap below to rotate or delete
                  </p>
                )}
              </Panel>
            </div>
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <Panel>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {selectedPage && (
                    <div className="flex gap-2">
                      <button
                        onClick={() => rotatePage(selectedPage.id)}
                        title="Rotate 90°"
                ...
                {selectedPage ? (
                  <div className="space-y-4">
                    <div
                      className="relative rounded-xl overflow-hidden flex items-center justify-center min-h-[340px]"
                    ...
                    <button
                      ...
                      style={{ background: "rgba(16,185,129,0.1)", border: `1px solid rgba(16,185,129,0.3)`, color: G.accentLight }}
                    >
                      {isEnhancing ? (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Slider
                          value={[selectedPage.brightness]} min={50} max={160} step={5}
                      ...
                      <div className="space-y-2">
                        ...
                        <Slider
                          value={[selectedPage.contrast]} min={50} max={160} step={5}
                ...
                {isDrawingSignature ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <OutlineBtn onClick={clearSignatureCanvas}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      background: pages.length > 0 ? "rgba(16,185,129,0.05)" : "rgba(255,255,255,0.02)"
                    }}
                    onClick={pages.length > 0 ? startSignatureDrawing : undefined}
                  >
                    <Pen className="w-9 h-9 mx-auto mb-3"
                      style={{ color: pages.length > 0 ? G.accentLight : "rgba(255,255,255,0.15)" }} />
                    <p className="text-sm font-semibold"
                      style={{ color: pages.length > 0 ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.25)" }}>
                      {pages.length > 0 ? "Click to Draw Signature" : "Add pages first to sign"}
                    </p>
                    <p className="text-xs mt-1" style={{ color: "rgba(255,255,255,0.25)" }}>
                      Mouse or touch supported
                    </p>
                  </div>
                )}
                {signatures.length > 0 && (
              ...
              {/* ── Export + Save Row — inside the outer border ── */}
              <div className="rounded-2xl p-4 space-y-3"
                ...
                <div className="flex gap-3">
                  <OutlineBtn onClick={saveProject} disabled={pages.length === 0} className="flex-1">
                    <Save className="w-4 h-4" /> Save Project
                  </OutlineBtn>
                  <DangerBtn onClick={clearProject} disabled={pages.length === 0} className="flex-1">
