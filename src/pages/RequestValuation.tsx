        ...
        <nav className="sticky top-0 lg:top-[48px] z-30 bg-white/90 backdrop-blur-md border-b border-[#C8A766]/15 shadow-sm">
          <div className="max-w-5xl mx-auto px-6 overflow-x-auto scrollbar-hide">
            <div className="flex gap-6 py-3 min-w-max">
              {anchors.map(a => (
                <a key={a.id} href={`#${a.id}`} className="text-sm text-gray-600 hover:text-[#C8A766] transition-colors whitespace-nowrap font-medium">
                  {a.label}
                </a>
              ))}
            </div>
          ...
          <section id="why" className="pt-16">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6">
                <h3 className="font-serif font-bold text-gray-600 text-lg mb-4">Informal Estimate</h3>
                {comparisonRows.map((r, i) => (
                  <p key={i} className="text-gray-600 text-sm py-2 border-b border-gray-100 last:border-0">{r.informal}</p>
                ))}
              </div>
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              {outputRows.map(r => (
                <div key={r.label} className="bg-white/80 border border-[#C8A766]/20 rounded-2xl p-6 shadow-sm text-center">
                  <p className="text-sm text-gray-600 mb-2">{r.label}</p>
