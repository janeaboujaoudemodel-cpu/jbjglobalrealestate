    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FDFBF7] via-[#F5F0E6] to-[#EDE4D3] flex">
      ...
      <aside className={`${sidebarCollapsed ? 'w-16' : 'w-64'} shrink-0 border-r-2 border-gold/30 bg-gradient-to-b from-black via-zinc-900 to-black transition-all duration-300 hidden lg:flex flex-col`}>
        ...
        <div className="p-4 border-b border-gold/20 flex items-center gap-3 h-16">
          {!sidebarCollapsed && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-gold font-bold text-sm">AI Command</span>
            </div>
          )}
          <button 
            onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
            className="ml-auto text-zinc-400 hover:text-gold transition-colors"
          >
        ...
        <ScrollArea className="flex-1">
          ...
          <div className="p-3">
            {!sidebarCollapsed && (
              <p className="text-[10px] font-bold text-gold/60 uppercase tracking-widest mb-3 px-2">AI Tools</p>
            )}
            <div className="space-y-1">
              {AI_TOOLS.map((tool) => (
                <Link
                  key={tool.route}
                  to={tool.route}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-zinc-300 hover:text-white hover:bg-white/10 transition-all group ${sidebarCollapsed ? 'justify-center' : ''}`}
                  title={tool.label}
                >
                  <tool.icon className={`w-4 h-4 ${tool.color} shrink-0`} />
                  {!sidebarCollapsed && <span className="text-xs font-medium truncate">{tool.label}</span>}
                </Link>
              ))}
            </div>
          </div>
          ...
          {/* Divider */}
          <div className="mx-3 h-px bg-gold/20 my-2" />

          {/* Admin Shortcuts */}
          <div className="p-3">
            {!sidebarCollapsed && (
              <p className="text-[10px] font-bold text-gold/60 uppercase tracking-widest mb-3 px-2">Quick Access</p>
            )}
            <div className="space-y-1">
              {ADMIN_SHORTCUTS.map((item) => (
                <Link
                  key={item.route}
                  to={item.route}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-zinc-400 hover:text-gold hover:bg-gold/10 transition-all ${sidebarCollapsed ? 'justify-center' : ''}`}
                  title={item.label}
                >
        ...
        <div className="p-3 border-t border-gold/20">
          <Link
            to="/toolkit"
            className={`flex items-center gap-2 px-3 py-2 rounded-lg bg-gold/10 border border-gold/30 text-gold hover:bg-gold/20 transition-all ${sidebarCollapsed ? 'justify-center' : ''}`}
          >
            <Sparkles className="w-4 h-4 shrink-0" />
            {!sidebarCollapsed && <span className="text-xs font-semibold">All 52+ Tools</span>}
          </Link>
        </div>
      </aside>
