...
const InquiryManagementHub: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FDFBF7] to-[#F7F2EA] pb-24">
            ...
            <TabsContent value={activeTab} className="mt-4">
              {isLoading ? (
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-xs text-gray-600 mt-1">{error instanceof Error ? error.message : 'Please try again.'}</p>
                </div>
              ) : filtered.length === 0 ? (
                <div className="text-center py-20">
                          ...
                          >
                            <TableCell>
                              <div>
                                ...
                                <p className="text-xs text-gray-600">{inq.email}</p>
                                {inq.phone && <p className="text-xs text-gray-600">{inq.phone}</p>}
                              </div>
                            </TableCell>
                            <TableCell>
                              <p className="font-medium text-black line-clamp-1">{inq.subject}</p>
                              {inq.message && <p className="text-xs text-gray-600 line-clamp-1 mt-0.5">{inq.message}</p>}
                            </TableCell>
                            <TableCell>
              ...
              <>
                ...
                <div className="space-y-5 mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 bg-gradient-to-br from-gold/5 to-gold/10 rounded-xl border border-gold/20">
                    ...
                    <div>
                      <p className="text-xs text-gray-600 uppercase tracking-wide mb-1">Details</p>
                      <div className="space-y-1 text-sm">
                        <p><span className="text-gray-600">Type:</span> <span className="capitalize">{(selectedInquiry.inquiry_type || 'general').replace(/_/g, ' ')}</span></p>
                        <p><span className="text-gray-600">Source:</span> {selectedInquiry.source || '—'}</p>
                  ...
                  {/* Admin Notes */}
                  <div>
                    <p className="text-xs text-gray-600 uppercase tracking-wide mb-2">Owner Notes</p>
                    <Textarea
                      value={adminNotes}
