...
const QuizResults = () => {
  ...
  /**
   * Single source of truth for the AI Home Finder PDF.
   * Renders the SAME <ReportEngine /> the Live Preview uses, then captures
   * it via html2canvas + jsPDF inside `renderReportToPdf`. There is no
   * second layout — if the preview and the PDF diverge, fix the React
   * component, never reintroduce hand-drawn jsPDF code here.
   */
  const buildPdf = async (
    branding?: ReportBranding
  ): Promise<{ blob: Blob; filename: string } | null> => {
    if (!projects?.length) return null;
    const resolvedBranding: ReportBranding = branding ?? {
      role: "consultant",
      mode: "none",
      website: "www.jbj.ae",
      address: "Dubai, UAE",
