      ...
      <div className="bg-gradient-to-b from-[#1a1714] to-[#151210] border-b border-[#C8A766]/20">
        <div className="max-w-[1200px] mx-auto px-6 pt-8 pb-16">
          ...
          <div className="max-w-3xl">
            ...
            <h1 className="text-white text-4xl md:text-5xl font-bold mb-4" style={{ fontFamily: "Playfair Display, Georgia, serif" }}>Cookie Policy</h1>
            <p className="text-[#C8A766] text-lg md:text-xl mb-6" style={{ fontFamily: "Playfair Display, Georgia, serif" }}>Transparency in How We Use Technology</p>
            <p className="text-zinc-400 leading-relaxed max-w-2xl">This Cookie Policy explains how our website uses cookies and similar technologies to enhance user experience, analyse performance, and deliver relevant services.</p>
            <p className="text-zinc-400 leading-relaxed max-w-2xl mt-3">By using our website, you consent to the use of cookies as described in this Policy.</p>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-12">
        <div className="lg:hidden mb-10 bg-[#1a1714]/80 backdrop-blur border border-[#C8A766]/20 rounded-xl p-6">
          ...
          <nav className="space-y-2">
            {tocItems.map(({ id, label }) => (<button key={id} onClick={() => scrollTo(id)} className="block w-full text-left text-sm text-zinc-500 hover:text-[#C8A766] transition-colors py-1">{label}</button>))}
          </nav>
        </div>
            ...
            <div className="sticky top-8">
              ...
              <nav className="space-y-1 border-l border-[#C8A766]/20">
                {tocItems.map(({ id, label }) => (
                  <button key={id} onClick={() => scrollTo(id)} className={`block w-full text-left pl-4 py-1.5 text-sm transition-all border-l-2 -ml-px ${activeSection === id ? "border-[#C8A766] text-[#C8A766] font-medium" : "border-transparent text-zinc-500 hover:text-zinc-300"}`}>{label}</button>
                ))}
              </nav>
            </div>
          </aside>
