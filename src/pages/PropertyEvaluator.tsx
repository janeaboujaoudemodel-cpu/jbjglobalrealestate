...
const PropertyEvaluator = () => {
  ...
  const buildSelectedReportText = () => {
    ...
    if (enabled('disclaimer')) {
      blocks.push(`SOURCE NOTES & DISCLAIMER\n${evaluation.sources || ''}\n${evaluation.disclaimer}\nReport generated by JBJ Global Real Estate · www.jbj.ae · CONTACT@JBJ.AE · +971 54 15 15 015`);
    }
    return blocks.join('\n\n════════════════════════════════════════\n\n');
  };
        ...
        {!entryChosen ? (
          <div className="max-w-5xl mx-auto py-6 w-full">
            <div className="text-center mb-8">
              <div className="jj-ai-gate-chip inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold mb-4">
                <Sparkles className="w-3.5 h-3.5 text-white" /> Start your valuation
              </div>
              <h2 className="jj-ai-gate-title text-2xl sm:text-3xl font-bold">How would you like to evaluate your property?</h2>
              <p className="jj-ai-gate-sub mt-2 text-sm sm:text-base">Choose one option. You can review and edit every field before generating the report.</p>
            </div>
              ...
              {[
                {
                  ...
                  title: 'Evaluate with Title Deed',
                  Icon: FileCheck,
                  desc: 'Upload your Title Deed or Oqood (PDF / JPG / PNG). AI extracts community, tower, unit, size, floor, view, handover year and owner — every field is pre-filled automatically.',
                  bullets: ['Auto-fills all property details', 'You review and edit before evaluating', 'Guided upload with instant preview'],
                  cta: 'Start with Title Deed',
                },
                {
                  key: 'manual' as const,
                  ...
                  Icon: Building,
                  desc: 'Enter your property specifications yourself — building, community, size, view, upgrades and photos. Best when you don\u2019t have the Title Deed handy.',
                  bullets: ['Type every field yourself', 'Full control over specifications', 'Switch to Title Deed later anytime'],
                  cta: 'Fill manually',
                },
              ].map(({ key, eyebrow, title, Icon, desc, bullets, cta }) => (
                <div
                  key={key}
                  role="button"
                  data-contained-card
                  ...
                  className="jj-ai-gate-card group relative w-full min-w-0 aspect-square sm:aspect-[1/1] rounded-[28px] p-6 sm:p-8 cursor-pointer flex flex-col justify-between overflow-hidden transition-transform hover:scale-[1.015] focus:outline-none focus:ring-2 focus:ring-emerald-400"
                  style={{
                    background: 'radial-gradient(120% 120% at 0% 0%, rgba(6,95,70,0.95) 0%, rgba(4,44,28,0.92) 45%, rgba(0,0,0,0.98) 100%)',
                ...
                >
                  <div>
                    ...
                    <div className="text-[11px] uppercase tracking-[0.18em] text-white/90 font-bold mb-2 whitespace-normal break-words [overflow-wrap:anywhere]">{eyebrow}</div>
