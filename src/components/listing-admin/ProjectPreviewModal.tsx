...
export function ProjectPreviewModal({ project, open, onOpenChange, onEdit, onOpenDetail, onSendToSarah }: ProjectPreviewModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 overflow-hidden bg-gradient-to-br from-[#FDFBF7] via-[#F7F2EA] to-[#EFE6D6] border-2 border-[#B89555]/30">
        ...
        <div className="relative h-64 bg-gradient-to-br from-muted to-muted/50 overflow-hidden">
          ...
          <div className="absolute top-3 left-3 flex gap-2">
            {project.is_premium && (
              <Badge className="bg-[#EFE6D6] text-foreground text-xs font-bold gap-1">
            ...
            {(project as any).construction_status && (
              <Badge variant="outline" className="text-xs bg-background/80 backdrop-blur-sm border-[#B89555]/30">
                {(project as any).construction_status}
              </Badge>
            )}
          </div>
        ...
        <div className="p-6 space-y-4">
          ...
          {/* Last Edit Info */}
          {lastEdit && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-background/60 border border-[#B89555]/20">
          ...
          {/* Actions */}
          <div className="flex flex-wrap gap-3 pt-2 border-t border-[#B89555]/20">
            {onOpenDetail && (
              <Button
                variant="primary"
