  ...
  return (
    ...
    >
      ...
      <Link
        ...
        className="oi-investor-card allow-white group relative block w-full overflow-hidden px-6 py-10 transition-colors duration-300 sm:px-10 md:px-16 md:py-14 lg:px-24 lg:py-16"
        style={{
          background:
      ...
      >
        <span aria-hidden className="oi-card-orb absolute left-10 top-10 h-32 w-32 rounded-full bg-[#B89555]/10 blur-3xl" />
        <span aria-hidden className="oi-card-orb absolute bottom-10 right-16 h-40 w-40 rounded-full bg-[#B89555]/10 blur-3xl" style={{ animationDelay: "1.4s" }} />

        <span className="relative z-[1] mx-auto flex w-full max-w-[1400px] flex-col gap-8 lg:grid lg:grid-cols-[1fr_auto] lg:items-end lg:gap-10">
          <span className="min-w-0">
            ...
            <span className="block max-w-[860px] text-[2rem] font-extrabold leading-[0.98] tracking-normal sm:text-[2.7rem] md:text-[4rem] lg:text-[4.4rem]" style={{ color: "#FFFFFF" }}>
        ...
        <span className="relative z-[1] mx-auto mt-8 block h-px w-full max-w-[1400px]" style={{ background: "linear-gradient(90deg, transparent, rgba(184,149,85,0.34), transparent)" }} />

        <span className="relative z-[1] mx-auto mt-7 grid w-full max-w-[1400px] grid-cols-2 gap-y-6 sm:grid-cols-4">
          {microStats.map((s, i) => {
            const Icon = s.icon;
            return (
