        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          {displayAreas.map((area, index) => (
            ...
            >
              <Link
                to={`/area/${area.slug}`}
                className="group relative block h-[200px] md:h-[220px] rounded-xl overflow-hidden border border-[#B89555]/30 hover:border-[#B89555]/30 transition-all duration-300 hover:shadow-lg hover:-translate-y-1.5"
              >
                ...
                {area.imageUrl ? (
                  ...
                  <div className="absolute inset-0 bg-[#F7F2EA] flex items-center justify-center">
                    <span
                      className="text-6xl font-black text-[#1A1A1A] select-none"
                      style={{ opacity: 0.1 }}
                    >
                      JBJ
                    </span>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3">
                  {area.propertyCount != null && area.propertyCount > 0 && (
                    <span className="inline-block mb-1.5 px-2 py-0.5 rounded-full bg-[#1A1A1A]/60 text-white text-[9px] font-semibold tracking-wide border border-white/20">
