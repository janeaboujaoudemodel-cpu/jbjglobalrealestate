...
import { 
  Heart, Target, Key, Home, User, Layers, Users, Briefcase, Globe, Award, GraduationCap,
  FileText, BarChart3, Scale, Calculator, Palette, Building2, ArrowRight
} from 'lucide-react';
...
const StartingPointSection = () => {
  ...
  const audienceCards = [
    ...
    { to: "/rent-guide", icon: Key, label: t('hero.rentals'), sub: t('hero.findRentals', 'Find Rentals') },
    ...
    { to: "/ai-hub", icon: Layers, label: t('hero.investors'), sub: t('hero.aiTools', 'AI Tools') },
  ...
  const professionalCards = [
    { to: "/broker-hub", icon: Building2, label: t('hero.jbjBrokerHub'), sub: t('hero.professionalTools', 'Professional Tools'), featured: true },
    { to: "/investor-hub", icon: Layers, label: t('hero.jbjInvestorHub'), sub: t('hero.freeAiTools', 'Free AI Tools'), featured: true },
  ...
  return (
    <section className="bg-black">
      <div className="jj-layer-2">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.key
                    ? "bg-gradient-to-r from-gold/90 to-gold text-black shadow-[0_4px_20px_rgba(200,167,102,0.4)]"
        ...
        {/* Cards grid — clean 2-col mobile, 4-col tablet, dynamic desktop */}
          ...
          {activeCards.map((card) => (
            <Link key={card.to} to={card.to} className="group">
              <div className={cn(
                ...
                'featured' in card && card.featured && "md:col-span-2 border-gold/60 shadow-[0_8px_30px_rgba(200,167,102,0.35)]"
              )}>
                {/* Hover glow */}
                ...
                <div className="relative flex flex-col items-center gap-2 md:gap-3 w-full">
                  <div className={cn(
                    "rounded-xl flex items-center justify-center group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(200,167,102,0.4)] transition-all duration-300 shadow-lg shrink-0",
                    'featured' in card && card.featured 
                      ? "w-14 h-14 bg-gradient-to-br from-gold/20 to-gold/10 border-2 border-gold/40" 
                      : "w-11 h-11 md:w-12 md:h-12 bg-black"
                  )}>
                    <card.icon className={cn(
                      "text-gold",
                      'featured' in card && card.featured ? "w-7 h-7" : "w-5 h-5"
                    )} strokeWidth={'featured' in card && card.featured ? 1.5 : 2} />
                  </div>
