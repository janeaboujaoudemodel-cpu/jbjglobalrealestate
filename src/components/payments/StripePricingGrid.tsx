...
/**
 * Stripe-wired pricing grid. Same visual language as PricingGrid but opens
 * the seamless Stripe embedded checkout instead of the manual payment-request
 * dialog. Use for products created via payments--batch_create_product.
 */
export function StripePricingGrid({
  ...
  const handleSelect = (tier: PricingTier) => {
    const price = tier.prices.find((p) => p.interval === interval) ?? tier.prices[0];
    if (!price) return;
    ...
    if (!user) {
      ...
      navigate(`/auth?next=/broker-pricing`);
      return;
    }
      ...
      <section className="jj-section py-16 md:py-24">
        <div className="mx-auto max-w-6xl px-4">
          ...
          {availableIntervals.length > 1 ? (
            <div className="mx-auto mb-10 inline-flex items-center gap-1 rounded-full border border-[#B89555]/40 bg-white p-1 shadow-sm w-fit">
                ...
                >
                  ...
                  <CardContent className="flex flex-col flex-1">
                    <div className="mb-4">
                      ...
                      {price.savingsNote ? (
                        <div className="text-xs text-[#064E3B] mt-1">{price.savingsNote}</div>
                      ) : null}
                    </div>
...
export default StripePricingGrid;
