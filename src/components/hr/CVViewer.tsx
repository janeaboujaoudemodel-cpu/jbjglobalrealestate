  ...
  const getSignedUrl = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.storage
        .from('hr-documents')
        .createSignedUrl(cvUrl, 3600); // 1 hour expiry
        ...
        <DialogHeader className="p-4 border-b border-border">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <DialogTitle className="text-xl text-white">{candidateName}</DialogTitle>
                <p className="text-sm text-muted-foreground">{candidateData?.position || 'Candidate'}</p>
              </div>
            </div>
              ...
              <Card className="bg-card border-border">
                ...
                <CardContent className="space-y-2">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full justify-start gap-2 text-green-500 border-green-500/50 hover:bg-green-500/10"
                ...
                <Card className="bg-purple-500/10 border-purple-500/30">
                  ...
                  <CardContent className="space-y-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-white text-xs">{candidateData.aiAnalysis.experience}</p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-white text-xs">{candidateData.aiAnalysis.education}</p>
                    </div>
                    ...
                    <div className="pt-2 border-t border-purple-500/30">
                      <span className="text-muted-foreground mb-1 block text-xs">AI Recommendation</span>
                      <p className="text-purple-300 text-xs italic">"{candidateData.aiAnalysis.recommendation}"</p>
                    </div>
                  </CardContent>
                </Card>
              )}
              ...
              <TabsContent value="preview" className="flex-1 overflow-hidden">
                {loading ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-lg font-semibold text-white mb-2">CV Not Available</h3>
                  ...
                  <CardContent className="pt-6">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-white">{candidateData?.position || 'Not specified'}</p>
                      </div>
                      <div>
                        <Label className="text-muted-foreground">Email</Label>
                        <p className="text-white">{candidateData?.email || 'N/A'}</p>
                      </div>
                      <div>
                        <Label className="text-muted-foreground">Phone</Label>
                        <p className="text-white">{candidateData?.phone || 'N/A'}</p>
                      </div>
                      <div>
                        <Label className="text-muted-foreground">Nationality</Label>
                        <p className="text-white">{candidateData?.nationality || 'N/A'}</p>
                      </div>
                      <div>
                      ...
                      <div className="col-span-2">
                        <Label className="text-muted-foreground">CV File</Label>
...
const Label = ({ className, children }: { className?: string; children: React.ReactNode }) => (
  <p className={`text-xs mb-1 ${className || ''}`}>{children}</p>
);
