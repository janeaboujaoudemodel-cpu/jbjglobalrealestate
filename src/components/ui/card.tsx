...
const cardVariants = cva(
  "min-w-0 max-w-full overflow-hidden rounded-lg border border-[#B89555]/30 bg-gradient-to-br from-[#FDFBF7] via-[#F7F2EA] to-[#EFE6D6] text-[#1A1A1A] shadow-sm transition-colors duration-200 transform-none hover:transform-none",
  {
    variants: {
      surface: {
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn(
        "min-w-0 max-w-full text-lg md:text-xl font-medium leading-tight tracking-tight text-foreground",
        className,
      )}
      {...props}
    />
...
const CardFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex min-w-0 max-w-full flex-wrap items-center gap-2 p-6 pt-4 border-t border-border/20", className)}
      {...props}
    />
  ),
);
