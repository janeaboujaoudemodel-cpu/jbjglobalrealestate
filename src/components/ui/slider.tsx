...
>(({ className, ...props }, ref) => (
  <SliderPrimitive.Root
    ...
    className={cn(
      ...
      // Use touch-pan-y to allow vertical scrolling while enabling horizontal drag
      "touch-pan-y",
      className
    )}
    {...props}
  >
    ...
    <SliderPrimitive.Thumb 
      className={cn(
        // Larger touch target for mobile (h-7 w-7 instead of h-5 w-5)
        "block h-7 w-7 rounded-full border-2 border-primary bg-background",
        "ring-offset-background transition-colors",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
        "disabled:pointer-events-none disabled:opacity-50",
        // Enable native touch manipulation for smoother dragging
