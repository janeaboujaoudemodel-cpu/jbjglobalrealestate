...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    items: [
     ...
     { label: "Data Gaps", icon: AlertTriangle, path: "/owner/data-gaps" },
      { label: "Market Import Review", icon: DatabaseZap, path: "/owner/crm/jbj/owner-market-import" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "CRM Directory", icon: Users, path: "/owner/crm-directory" },
...
export default function OwnerSidebarNav({ collapsed, onNavigate }: OwnerSidebarNavProps) {
  ...
  return (
    <nav data-no-contrast-guard data-surface="light" className="flex-1 overflow-y-scroll jj-scrollbar-gold py-3 px-2 space-y-1" style={{ scrollbarGutter: "stable" }}>
      {NAV_SECTIONS.map((section) => (
        <div key={section.label} className="mb-1.5 last:mb-0">
          {!collapsed && (
            <p className="text-[10px] uppercase tracking-[0.16em] text-[#1A1A1A]/58 font-black px-3 mb-1.5 mt-2 first:mt-0">
              {section.label}
            </p>
          )}
