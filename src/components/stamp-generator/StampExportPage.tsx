...
import {
  ...
  FileImage, FileText, File, Package, Palette, X, Plus
} from 'lucide-react';

// ─── Helpers ──────────────────────────────────────────────────────────────────

/** Reliable SVG → PNG using Blob URL + canvas */
...
/** Apply tint color to SVG string */
function tintSvg(svgString: string, color: string): string {
  return svgString.replace(/#1a2744/gi, color);
}
...
export default function StampExportPage() {
  ...
  const [exportColor, setExportColor] = useState('#1a2744');
  ...
  const tintedSvg = design?.svg_source ? tintSvg(design.svg_source, exportColor) : '';
  ...
  async function downloadPNGFile(size: number) {
    const key = `png_${size}`;
    try {
    ...
    try {
      ...
      for (const size of options.sizes) {
        if (options.formats.includes('png')) {
          const ok = await downloadPNGFile(size);
      ...
      if (failCount === 0) {
        toast.success(`✅ ${successCount} file${successCount !== 1 ? 's' : ''} downloaded successfully!`);
      } else {
        toast.warning(`Downloaded ${successCount} files. ${failCount} failed — try downloading individually.`);
      }
    } catch (err) {
      console.error('Bundle error:', err);
      toast.error('Export encountered an error. Try individual file downloads.');
    }
    setGenerating(false);
  }
  ...
  async function downloadColorZip() {
    ...
    try {
      ...
      for (const { label, hex } of packColors) {
        ...
        const coloredSvg = tintSvg(design.svg_source, hex);
      ...
      toast.error('Failed to generate color pack. Please try again.');
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-[hsl(var(--border))] p-8 flex items-center justify-center">
              <StampSVGRenderer svgSource={design.svg_source} size={260} tintColor={exportColor}/>
            </div>

            {/* On document */}
            <div className="bg-white rounded-2xl border border-[hsl(var(--border))] p-4 space-y-2">
              <p className="text-xs font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wide">Preview on Document</p>
              <div className="bg-gradient-to-br from-[hsl(var(--pearl-1))] to-white rounded-xl p-5 border border-[hsl(var(--border)/0.5)] relative overflow-hidden">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="space-y-1">
                    <div className="h-2.5 w-28 bg-[hsl(var(--muted))] rounded"/>
                    <div className="h-1.5 w-40 bg-[hsl(var(--muted)/0.7)] rounded"/>
                    <div className="h-1.5 w-32 bg-[hsl(var(--muted)/0.5)] rounded"/>
                  </div>
                ...
                <div className="mt-6 flex items-end justify-between">
                  ...
                  <div className="opacity-90">
                    <StampSVGRenderer svgSource={design.svg_source} size={88} tintColor={exportColor}/>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-[hsl(var(--border))] p-4 space-y-2">
              ...
              <div className="relative bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-4 overflow-hidden" style={{ aspectRatio: '1.75 / 1' }}>
                {/* Card content */}
                <div className="absolute inset-0 flex items-center p-5 gap-4">
                  <StampSVGRenderer svgSource={design.svg_source} size={70} tintColor={exportColor}/>
                  <div className="flex-1 space-y-1">
                    <div className="h-2 w-24 bg-white/30 rounded"/>
                    <div className="h-1.5 w-16 bg-white/20 rounded"/>
                    <div className="h-1 w-20 bg-white/15 rounded mt-2"/>
          ...
          <div className="space-y-5">
            ...
            {/* Export Color — full wheel */}
            <div className="bg-white rounded-2xl border border-[hsl(var(--border))] p-5">
              <p className="text-sm font-semibold text-[hsl(var(--foreground))] mb-3 flex items-center gap-1.5">
                <Palette size={14} className="text-[hsl(var(--gold))]"/> Export Color
              </p>
              <StampColorWheel color={exportColor} onChange={setExportColor} label="" size={180}/>
            </div>
              ...
              {multiColorMode && (
                <div className="space-y-3">
                  ...
                  <p className="text-[10px] text-[hsl(var(--muted-foreground))]">
                    ZIP includes SVG + 512px + 1024px PNG for each color. Transparent background.
                  </p>
                </div>
              )}
            </div>
