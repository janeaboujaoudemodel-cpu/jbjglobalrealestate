...
const MEGA_MENU_LINKS: Record<MegaMenuKey, Array<{ label: string; href: string; icon: any }>> = {
  ...
  'ai-tools': [
    ...
    { label: 'List Property for Sale', icon: ClipboardCheck, href: '/listing-portal?type=sale' },
    { label: 'List Property for Rent', icon: Key, href: '/listing-portal?type=rent' },
  ],
  creative: [
    { label: 'AI Home Finder', icon: Home, href: '/quiz' },
    { label: 'Property Comparison', icon: GitCompare, href: '/compare' },
    ...
    <div className="flex flex-col h-full">
      ...
      >
        ...
        <div className="px-2.5 pt-3 pb-1.5">
          ...
          >
            <div className={`w-6 h-6 rounded-lg flex items-center justify-center ${activeMegaMenu === 'shortcuts' ? 'bg-[#EFE6D6]/20' : 'bg-[#EFE6D6]/10'}`}>
              <Zap className="w-3.5 h-3.5 text-[#1A1A1A]" />
            </div>
            <span className="flex-1 text-left tracking-wide">My Shortcuts</span>
            <ChevronRight className={`w-3 h-3 transition-transform duration-200 ${activeMegaMenu === 'shortcuts' ? "rotate-90 text-[#1A1A1A]" : "text-[#1A1A1A]/70"}`} />
          </button>
        </div>
