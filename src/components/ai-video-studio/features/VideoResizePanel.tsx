  ...
  return (
    <div className="h-full flex flex-col bg-[#1A1A1A] text-white">
      ...
      <div className="p-3 border-b border-[#1A1A1A]">
        ...
        <p className="text-xs text-[#1A1A1A]/70 mt-0.5">Select formats — applies to the active clip on your timeline</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
          ...
          <div className="space-y-1.5">
            {EXPORT_FORMATS.map((format) => (
              ...
              >
                ...
                />
                <format.icon className={`h-4 w-4 ${selectedFormats.includes(format.id) ? 'text-[#1A1A1A]' : 'text-[#1A1A1A]/70'}`} />
                <div className="flex-1">
                  <p className="text-xs font-semibold text-white">{format.name}</p>
                  <p className="text-[10px] text-[#1A1A1A]/70">{format.aspect} · {format.width}×{format.height}px</p>
                </div>
                {selectedFormats.includes(format.id) && (
        ...
        <div className="flex items-center justify-between p-3 bg-[#1A1A1A] rounded-lg border border-[#1A1A1A]">
          <div>
            <Label className="text-xs font-semibold text-white">Smart Framing</Label>
            <p className="text-[10px] text-[#1A1A1A]/70 mt-0.5">AI keeps subject centered when cropping</p>
          </div>
        ...
        <div>
          ...
          {!uploadedVideo ? (
            ...
            >
              <Upload className="h-5 w-5 text-[#1A1A1A]/70 mx-auto mb-1.5" />
        ...
        {/* Processing Progress */}
        {isLoading && progress && (
          <div className="p-3 bg-[#1A1A1A] rounded-lg border border-[#1A1A1A] space-y-2">
            <div className="flex items-center gap-2">
        ...
        {resizeJobs.length > 0 && (
          <div className="space-y-1.5">
            <h4 className="text-[10px] font-semibold text-[#1A1A1A]/70 uppercase tracking-widest">Results</h4>
            {resizeJobs.map((job) => (
              <div
                key={job.id}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-[#1A1A1A]/70">{job.targetWidth}×{job.targetHeight}</span>
                  </div>
