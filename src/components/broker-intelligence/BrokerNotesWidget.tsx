...
export function BrokerNotesWidget() {
  ...
  const getCategoryBadge = (category: QuickNote["category"]) => {
    switch (category) {
      case "lead":
        return <Badge className="jj-surface-emerald-soft text-emerald-400 border-[color:var(--emerald-1)]/30/30 text-xs"><User className="w-3 h-3 mr-1" />Lead</Badge>;
      case "property":
        ...
        <CardContent className="space-y-4">
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="bg-[#1A1A1A]/50 border-[#1A1A1A] text-white placeholder:text-[#1A1A1A]/70 pl-10"
            />
          </div>
          ...
          {filteredNotes.length === 0 ? (
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
              {filteredNotes.map((note) => (
                <div
                  key={note.id}
                  className="bg-[#1A1A1A]/50 rounded-lg p-3 border border-[#1A1A1A]/50 hover:border-[#B89555]/20 transition-colors group"
                >
                  ...
                  {note.linkedTo && (
                    <div className="flex items-center gap-1 mt-2 text-xs text-[#1A1A1A]">
