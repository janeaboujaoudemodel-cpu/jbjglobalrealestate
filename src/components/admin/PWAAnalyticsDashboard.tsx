    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Tabs value={dateRange} onValueChange={(v) => setDateRange(v as typeof dateRange)}>
            <TabsList className="bg-[#FDFBF7]/80 border-2 border-[#B89555]/30">
              <TabsTrigger value="7d" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#F7F1E6] data-[state=active]:via-[#ECE2D2] data-[state=active]:to-[#D8C7A6] data-[state=active]:border-[#B89555]/40 text-[#1A1A1A]">7 Days</TabsTrigger>
              <TabsTrigger value="30d" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-[#F7F1E6] data-[state=active]:via-[#ECE2D2] data-[state=active]:to-[#D8C7A6] data-[state=active]:border-[#B89555]/40 text-[#1A1A1A]">30 Days</TabsTrigger>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-[#FDFBF7] via-[#F7F2EA] to-[#EFE6D6] border-2 border-[#B89555]/30 shadow-[0_4px_20px_rgba(200,167,102,0.15)]">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-[#1A1A1A]">{totalClicks}</div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              {eventCounts.length === 0 && (
                <p className="text-[#1A1A1A]/50 text-center py-4">No events recorded yet</p>
              )}
            </div>
          </CardContent>
        </Card>
                ...
                <div className="space-y-2">
                  {deviceBreakdown.map((device) => (
                    <div key={device.device_type} className="flex items-center justify-between p-2 bg-[#FDFBF7]/60 border border-[#B89555]/20 rounded-lg">
                      ...
                      <span className="text-[#1A1A1A]/70 font-medium">{device.count}</span>
                    </div>
                  ))}
                  {deviceBreakdown.length === 0 && (
                    <p className="text-[#1A1A1A]/50 text-center py-4">No device data yet</p>
                  )}
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  {platformBreakdown.length === 0 && (
                    <p className="text-[#1A1A1A]/50 text-center py-4">No platform data yet</p>
                  )}
                </div>
              )}
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b-2 border-[#B89555]/20">
                  ...
                  <th className="text-left text-[#1A1A1A]/60 py-3 font-medium">Browser</th>
                  <th className="text-left text-[#1A1A1A]/60 py-3 font-medium">Time</th>
                </tr>
              </thead>
              <tbody>
                {analytics.slice(0, 20).map((row) => (
                  <tr key={row.id} className="border-b border-[#B89555]/10 hover:bg-[#FDFBF7]/40 transition-colors">
                    ...
                    <td className="py-3 text-[#1A1A1A]/70 capitalize">{row.browser || '-'}</td>
                    <td className="py-3 text-[#1A1A1A]/50">{format(new Date(row.created_at), 'MMM d, HH:mm')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {analytics.length === 0 && (
              <p className="text-[#1A1A1A]/50 text-center py-8">No analytics data yet</p>
            )}
          </div>
        </CardContent>
      </Card>
