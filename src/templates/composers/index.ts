...
function composeJobOffer(input: ComposerInput): string {
  ...
  const backdatedClause = leadsFromHuman
    ? `The Parties expressly acknowledge that the Candidate has been receiving Company leads, prospects, client data, owner/developer contacts, and confidential information from <strong>${esc(leadsFromHuman)}</strong>, in advance of the formal signing of this offer on <strong>${esc(signingHuman || "the date stated above")}</strong>. Accordingly, the Candidate's confidentiality, non-circumvention, data-protection, lead-ownership, non-solicitation, and indemnity obligations under this offer (and under the related Non-Disclosure Agreement and Employment Agreement) shall apply <strong>retroactively from ${esc(leadsFromHuman)}</strong> and shall continue in full force from that date until written notice from the Company expressly releases the Candidate from those obligations. Any leads, data, files, messages, or materials received by the Candidate from <strong>${esc(leadsFromHuman)}</strong> onwards shall be treated as Company property and governed by the protections set out in this offer.`
    : "";
