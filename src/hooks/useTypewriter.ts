...
export function useTypewriter(
  phrases: string[],
  opts: { typeMs?: number; eraseMs?: number; pauseEndMs?: number; pauseStartMs?: number } = {},
): string {
  const { typeMs = 55, eraseMs = 32, pauseEndMs = 1500, pauseStartMs = 350 } = opts;
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(t);
  }, [text, deleting, idx, phrases, typeMs, eraseMs, pauseEndMs, pauseStartMs]);
