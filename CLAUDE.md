...
**Item IDs (`JBJ-###`) and Changelog IDs (`RM-###`) — allocate from `NEXT_IDS.md`, not by grepping the table.** `NEXT_IDS.md` at the repo root (`JBJ_NEXT=###` / `RM_NEXT=###`) is the single source of truth for the next ID of each kind — read it, use the value it gives you, then increment that value and commit the bump as part of the same PR. Do **not** assign an ID by scanning the status table or changelog for the current highest number and adding one — under concurrent work (multiple sessions/PRs open against `main` at once), two sessions computing "highest + 1" independently will compute the same ID and collide, which is exactly the pattern that happened repeatedly during concurrent PRs in Aug 2026 (JBJ-024, RM-029, RM-030, RM-031 were each independently claimed by two PRs in the same ~25-minute window, forcing multiple rebase-and-renumber cycles before any of them could merge). `NEXT_IDS.md` fixes this only if every PR actually reads-and-bumps it — if your PR touches `ROADMAP.md`'s item table or changelog, `NEXT_IDS.md` must be part of the same diff. If you still hit a merge conflict on `NEXT_IDS.md` or on the ID you took (another PR merged first with the same number), rebase, take the next free number, and bump `NEXT_IDS.md` again — don't just resolve the text conflict and keep the stale number.

Item IDs are assigned once and never reused or renumbered once merged to `main`. Reference items by ID in commit messages and PR descriptions (`fixes JBJ-009`) instead of re-describing them.

**Changelog (`RM-###`).** Every edit to `ROADMAP.md` gets a row in the Changelog table, newest at the top, using the next `RM_NEXT` value from `NEXT_IDS.md` per the allocation rule above. Columns: ID, date (UTC — use the actual current date, not a guess), author, one-line description, item ID(s) touched, commit/PR reference. For "author," be honest about what actually made the change — "Claude Code session (PR #N)" is correct when that's what happened; don't attribute an automated or session-driven change to a person who didn't make it. Use `pending` for the commit column if you're adding the entry before the commit exists yet (e.g. mid-PR) — don't leave it blank.
