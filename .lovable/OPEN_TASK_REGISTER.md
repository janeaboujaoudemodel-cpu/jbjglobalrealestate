...
11. `DONE` Guest chat insert fails: `POST /rest/v1/chat_conversations` → 42501, anon has no grant/policy for guest chat inserts.
12. `DONE` `ai-chat-support` returns 401 (deployed copy out of sync with `verify_jwt = false`); `handleSend`'s catch path never persists the transcript, so the visitor's message is lost.
