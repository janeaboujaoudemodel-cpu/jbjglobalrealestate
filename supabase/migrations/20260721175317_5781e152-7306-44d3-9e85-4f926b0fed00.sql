...
SET html = REPLACE(
  html,
  'If your team runs an internal <strong>WhatsApp group</strong> for project updates, kindly add <strong>+974 15 15 015</strong> so we can keep {{brokerage_name}} informed on launches, inventory and commissions — otherwise we will happily create a dedicated group for your team.',
  'If your team already runs an internal <strong>WhatsApp group</strong> for project updates and briefings, and {{brokerage_name}} is <strong>not currently receiving the latest launches, inventory updates or support</strong> from CITI Developers, we would love to be added — kindly include <strong>+974 15 15 015</strong>. Otherwise, we will happily create a dedicated group for your team.'
),
updated_at = now()
