...
-- RPC 1: owner update from the Shared workspace.
-- Sets crm.context='shared' inside the same transaction as the UPDATE so the
-- trigger fans the diff out to brokers immediately.
...
BEGIN
  ...
  UPDATE public.crm_leads
     SET status         = COALESCE(p_patch->>'status', status),
         stage          = COALESCE(p_patch->>'stage', stage),
...
GRANT EXECUTE ON FUNCTION public.crm_update_lead_as_shared(uuid, jsonb) TO authenticated;

-- RPC 2: bundle for the Broker Profile hub
...
GRANT EXECUTE ON FUNCTION public.crm_broker_profile_bundle(uuid) TO authenticated;
