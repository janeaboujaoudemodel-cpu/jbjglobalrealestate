...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [sessions, events, daily, crmProfile, baseProfile, roles, activity, preferences] = await Promise.all([
    ...
    const synthesizedProfile = crmProfile.data || (profile ? {
      user_id,
      category: normalizeCategory(profile.picked_role) || normalizeCategory(pref?.selected_mode) || null,
