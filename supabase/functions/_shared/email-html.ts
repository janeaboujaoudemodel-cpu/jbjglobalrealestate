...
export function doNotReplyNotice(): string {
  return `<tr><td style="padding:20px 0 24px;text-align:center;"><p style="margin:0;font-size:11px;color:#8a8a8a;line-height:1.7;">This is an automated message. Please do not reply directly to this email.<br/>For any inquiries, contact us at <a href="mailto:CONTACT@JBJ.AE" style="color:#C8A766;text-decoration:underline;font-weight:600;">CONTACT@JBJ.AE</a></p></td></tr>`;
}

/**
 * LOCKED FOOTER — Social links with gold champagne icons on black.
 */
function socialLinksFooter(): string {
  const links = [
    { label: "Instagram", icon: SVG_ICONS.instagram, url: "https://www.instagram.com/jbj.ae" },
    { label: "Facebook", icon: SVG_ICONS.facebook, url: "https://www.facebook.com/share/1G7CgSaV2L/?mibextid=wwXIfr" },
    { label: "LinkedIn", icon: SVG_ICONS.linkedin, url: "https://www.linkedin.com/company/jbj-global-real-estate/" },
    { label: "YouTube", icon: SVG_ICONS.youtube, url: "https://youtube.com/@jbjglobalrealestate" },
    { label: "TikTok", icon: SVG_ICONS.tiktok, url: "https://www.tiktok.com/@jbj.ae" },
  ];

  const item = (label: string, icon: string, url: string) =>
    `<td style="padding:0 4px 8px;"><a href="${url}" style="display:inline-block;background:#000;border:1px solid #C8A766;border-radius:8px;padding:7px 10px;text-decoration:none;"><table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;"><tr><td style="padding-right:6px;vertical-align:middle;">${icon}</td><td style="color:#C8A766;font-size:10px;font-weight:700;letter-spacing:0.4px;vertical-align:middle;">${label}</td></tr></table></a></td>`;

  return `<table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin-bottom:10px;"><tr>${links.map((l) => item(l.label, l.icon, l.url)).join("")}</tr></table>`;
}
