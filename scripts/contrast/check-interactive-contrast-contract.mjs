...
const LIGHT_BG_RE = /(?<![:\w-])(bg-\[#(?:FDFBF7|F7F2EA|F7F1E6|EFE6D6|ECE2D2|B89555|A68444|D4AF37)\](?:\/[0-9]{1,3})?|bg-white|bg-background|bg-card|bg-popover|bg-secondary|bg-accent|bg-muted|from-\[#(?:FDFBF7|F7F2EA|F7F1E6|EFE6D6|ECE2D2)\])/;
...
function check() {
  ...
  for (const file of walk(SRC)) {
    ...
    lines.forEach((line, idx) => {
      ...
      const textRisk = (LIGHT_BG_RE.test(line) && WHITE_TEXT_RE.test(line)) || (DARK_BG_RE.test(line) && DARK_TEXT_RE.test(line)) || BAD_ACTIVE_TAB_RE.test(line);
      if (!textRisk) return;
      const nearby = lines.slice(Math.max(0, idx - 2), Math.min(lines.length, idx + 3)).join(' ');
      if (!INTERACTIVE_RE.test(nearby) && !/className=|className\{|Badge|TabsTrigger|role=/.test(nearby)) return;
