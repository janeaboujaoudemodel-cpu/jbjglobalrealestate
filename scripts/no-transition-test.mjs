...
console.log(await page.locator('button[data-emerald-action="true"]').first().evaluate(el=>{const before=getComputedStyle(el).color; el.classList.remove('transition-all'); el.classList.remove('duration-200'); return {before, after:getComputedStyle(el).color, transition:getComputedStyle(el).transition}}));
await browser.close();
