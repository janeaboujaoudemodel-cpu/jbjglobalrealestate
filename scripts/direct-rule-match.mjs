...
console.log(await page.evaluate(()=>{
 ...
 let out=[]; for(const ss of [...document.styleSheets]){try{for(const r of [...ss.cssRules]){if(r.selectorText?.includes('button[aria-selected') && r.style?.getPropertyValue('color')){let m=false,e=''; try{m=el.matches(r.selectorText)}catch(err){e=err.message} out.push({m,e,color:r.style.getPropertyValue('color'),prio:r.style.getPropertyPriority('color'),hasDesc:r.selectorText.includes(':where('), sel:r.selectorText.slice(0,1000)});}}}catch{}}
