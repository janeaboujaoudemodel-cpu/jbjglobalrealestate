...
console.log(await page.locator('button[data-emerald-action="true"]').first().evaluate(el=>{const before=getComputedStyle(el).color; el.style.setProperty('color','#fff','important'); el.style.setProperty('-webkit-text-fill-color','#fff','important'); return {before, immediate:getComputedStyle(el).color, inline:el.getAttribute('style'), cssText:el.style.cssText}}));
await page.waitForTimeout(1000);
console.log('later', await page.locator('button[data-emerald-action="true"]').first().evaluate(el=>getComputedStyle(el).color));
await browser.close();
